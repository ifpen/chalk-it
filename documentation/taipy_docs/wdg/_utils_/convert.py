...
def write_python_file(filename, datanodes):
    ...
    for node in datanodes:
        if node["type"] == "JSON_var_plugin":
            ...
            python_variables_code += (
                f"{variable_name} = {json.dumps(json.loads(json_var), indent=2)}\n\n"
            )
...
def remove_datanodes(filename, xprjson):
    ...
    if "libraries" in xprjson:
        xprjson["libraries"] = {"pyodideStandard": [], "pyodideMicropip": []}
    ...
    with open(new_file_path, "w") as file:
        json.dump(
            xprjson, file, indent=4
        )  # You can adjust the indent as per your needs
