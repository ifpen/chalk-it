...
angular.module('modules.python-images', [])
    .config(['$stateProvider',
        function($stateProvider) {
            $stateProvider
                .state('modules.python-images', {
    ...
    .service('PythonImagesManager', ['$http', '$rootScope', 'ApisFactory', function($http, $rootScope, ApisFactory) {
        ...
        this.createImage = async(image, fileInfo, progressCallback) => {
            ...
            FileMngrInst.AddPythonInfo(image.Id, responseCallback, image.BaseImage, "", image.Name, image.Description, image.FileName, image.FileSize);
        };
        ...
        this.updateImage = async(image, imageOld, fileInfo = {}, progressCallback) => {
            ...
            var FileMngrInst = new FileMngrFct();
            FileMngrInst.AddPythonInfo(image.Id, responseCallback, image.BaseImage, image.Hash, image.Name, image.Description, image.FileName, image.FileSize);
            const pythonImages = await _listPythonFiles();
            if (!pythonImages) {
        ...
        function _listFiles() {
            return new Promise((resolve, reject) => {
                var FileMngrInst = new FileMngrFct();
                FileMngrInst.GetFileList("pydata", (result, msg, type) => {
                    if (type === "success") {
        ...
        function _saveFileData(image, content, progressCallback) {
            ...
            return new Promise((resolve, reject) => {
                var FileMngrInst = new FileMngrFct();
                FileMngrInst.SendText("pydata", id, content, (result, msg, type) => {
                    if (type === "success") {
                        resolve(result);
                    } else {
                        reject(result);
        ...
        this.openOutputDialog = async(id, okCallback) => {
            ...
            function updateText(result) {
                ...
                if (result.status === "building") {
                    setTimeout(async() => updateText(await pythonImagesManager.getImageFullStatus(id)), OUTPUT_REFRESH_INTERVAL);
                }
            }
        ...
        function createEditForm(image, fileInfo) {
            ...
            const nameInput = $('<input type="text">').appendTo(nameRow).change(function() {
                image.Name = $(this).val();
            ...
            const descInput = $('<textarea class="calculated-value-input">').appendTo(descRow).change(function() {
            ...
            const fileInput = $(`<input type="text" id="select_file_path" value="${origFile}"></input><label class="board-toolbar datasource-input-suffix"><i class="fa-folder-open fa" aria-hidden="true"></i>Browse...</label>`);
            const fileDialog = $('<input type="file" style="display: none" id="select_file" accept=".zip, .txt"></input>').appendTo(fileInput);
