...
angular.module('modules.python-images')
    .config(['$stateProvider', function($stateProvider) {
        ...
        $stateProvider
            .state('modules.python-images.list', {
                ...
                resolve: {
                    _images: ['PythonImagesManager', '$rootScope', function(PythonImagesManager, $rootScope) {
...
.controller('PythonImagesListControler', ['$scope', '$rootScope', '$state', '$http', '$window', 'PythonImagesManager', '_images',
    function($scope, $rootScope, $state, $http, $window, PythonImagesManager, _images) {
        ...
        $scope.applyImgEditor = function() {
            ...
            if (_.isUndefined(image)) // at creation
                image = $scope.img;
            ...
            if ($scope.imgEditorBtn == 'Create') {
                ...
                PythonImagesManager.createImage($scope.img, $scope.fileInfo, (p) => setProgress($scope.img.Id, p))
                    .finally(() => {
                ...
                PythonImagesManager.updateImage(image, imageOld, fileInfo, (p) => setProgress(image.Id, p))
                    .finally(() => {
