﻿ // ┌───────────────────────────────────────────────────────────────────────────────────┐ \\
 ...
 angular.module('modules.editor')
     .controller('EditorController', ['$scope', '$rootScope', '$state', '$timeout', 'UndoManagerService', 'EditorActionFactory', 'WidgetContainerGetter', 'WidgetEditorGetter', 'EventCenterService', 'DashboardActiveTabGetter', 'DashboardActiveModeGetter',
         function($scope, $rootScope, $state, $timeout, undoManagerService, editorActionFactory, widgetContainerGetter, widgetEditorGetter, eventCenterService, dashboardActiveTabGetter, dashboardActiveModeGetter) {
             ...
             const _selectionCallback = () => $timeout(() => { // Protects us from being called from ouside of angular
             ...
             vm.updateBorderInWidgets = function() {
                 if (vm.borderInWidgets)
                     $("#DropperDroite").addClass("show-widget-borders");
                 else
                     $("#DropperDroite").removeClass("show-widget-borders");
             };

             vm.updateShowGrid = function() {
                 if (vm.showGrid)
                     $("#DropperDroite").addClass("show-grid");
                 else
                     $("#DropperDroite").removeClass("show-grid");
             };
             ...
             function _onkeydown(e) {
                 ...
                 if (e && e.target && (e.target instanceof HTMLInputElement || e.target instanceof HTMLSelectElement || e.target instanceof HTMLTextAreaElement)) {
             ...
             vm.alignTop = function _alignTop() {
                 ...
                 if (elementIds && elementIds.length > 1) {
                     ...
                     const action = editorActionFactory.createAlignementTopAction(target, elementIds.filter(it => it !== target));
                     undoManagerService.execute(action);
                 }
             };
             ...
             vm.alignBottom = function _alignBottom() {
                 ...
                 if (elementIds && elementIds.length > 1) {
                     const target = elementIds[elementIds.length - 1];
                     const action = editorActionFactory.createAlignementBottomAction(target, elementIds.filter(it => it !== target));
                     undoManagerService.execute(action);
                 }
             };
             ...
             vm.alignHorizontal = function _alignHorizontal() {
                 ...
                 if (elementIds && elementIds.length > 1) {
                     const target = elementIds[elementIds.length - 1];
                     const action = editorActionFactory.createAlignementHorizontalAction(target, elementIds.filter(it => it !== target));
                     undoManagerService.execute(action);
                 }
             };
             ...
             vm.alignLeft = function _alignLeft() {
                 ...
                 if (elementIds && elementIds.length > 1) {
                     const target = elementIds[elementIds.length - 1];
                     const action = editorActionFactory.createAlignementLeftAction(target, elementIds.filter(it => it !== target));
                     undoManagerService.execute(action);
                 }
             };
             ...
             vm.alignVertical = function _alignVertical() {
                 ...
                 if (elementIds && elementIds.length > 1) {
                     const target = elementIds[elementIds.length - 1];
                     const action = editorActionFactory.createAlignementVerticalAction(target, elementIds.filter(it => it !== target));
                     undoManagerService.execute(action);
                 }
             };
             ...
             vm.alignRight = function _alignRight() {
                 ...
                 if (elementIds && elementIds.length > 1) {
                     const target = elementIds[elementIds.length - 1];
                     const action = editorActionFactory.createAlignementRightAction(target, elementIds.filter(it => it !== target));
                     undoManagerService.execute(action);
                 }
             };
             ...
             function _clickOnDataConnection(check) {
                 ...
                 if (check) {
                     if ( /*scopeDash.editorView.rightSidePanel.target == "DataNode Connection" &&*/ scopeDash.editorView.rightSidePanel.view) {
                         modalCtrl.ensureSavedOrDiscarded()
             ...
             vm.editDatanodeCode = function _editDatanodeCode() {
                 ...
                 if (!_.isUndefined(cnxs[elementIds[0]])) {
                     _.each(_.keys(cnxs[elementIds[0]].sliders), (slider) => {
                         ...
                         if (connectedDataNodeS.indexOf(dsName) === -1) { //fix issue#23
             ...
             function _withGeometryAndElementIds(fct) {
                 ...
                 if (widgetEditorGetter && widgetContainerGetter &&
                     geom.top !== null && geom.top !== undefined &&
                     geom.left !== null && geom.left !== undefined &&
                     geom.height !== null && geom.height !== undefined &&
                     geom.width !== null && geom.width !== undefined) {
             ...
             vm.positionLeftChange = function _positionLeft(canMerge = true) {
                 return _doWithGeometryAndElementIds((geom, elementIds) => editorActionFactory.createSetWidgetLeftAction(elementIds, geom.left, canMerge));
             };

             vm.positionTopChange = function _positionTop(canMerge = true) {
                 return _doWithGeometryAndElementIds((geom, elementIds) => editorActionFactory.createSetWidgetTopAction(elementIds, geom.top, canMerge));
             };

             vm.sizeWidthChange = function _sizeWidth(canMerge = true) {
                 return _doWithGeometryAndElementIds((geom, elementIds) => editorActionFactory.createSetWidgetWidthAction(elementIds, geom.width, canMerge));
             };

             vm.sizeHeightChange = function _sizeHeight(canMerge = true) {
                 return _doWithGeometryAndElementIds((geom, elementIds) => editorActionFactory.createSetWidgetHeightAction(elementIds, geom.height, canMerge));
             };
