...
class UpdateGeometryAction extends UndoableAction {
     ...
     */
    static CreateSimpleUpdate(widgetContainer, widgetEditor, eventCenter, elementIds, geometryUpdate, label, relative = false, mergeTimeoutMs) {
        const updates = new Map();
        for (const id of elementIds) {
            updates.set(id, geometryUpdate);
        }
        return new UpdateGeometryAction(widgetContainer, widgetEditor, eventCenter, updates, label, relative, mergeTimeoutMs);
    }
    ...
    merge(previous) {
        if (previous instanceof UpdateGeometryAction &&
            previous._mergeTimeout !== undefined && Date.now() <= previous._mergeTimeout &&
...
class CreateWidgetAction extends UndoableAction {
    ...
    redo() {
        const widget = this._widgetEditor.addWidget(this._modelJsonId, this._getContainerDivOrId(), this._widgetId, this._layout);
        this._widgetId = widget.instanceId;
    }
    ...
    undo() {
        for (const data of this._widgetsData) {
            ...
            if (data.widgetsConnection) {
                ...
                this._widgetConnector.updateDataNodeIndex(data.id);
            }
        }
    }
}

function _postChangeUpdates(widgetEditor, widgetConnector, eventCenterService, elementIds, highlight = false, bCaptionManuallyChanged) {
    for (const elementId of elementIds) {
        widgetConnector.updateDataNodeIndex(elementId);
...
class SetWidgetParametersAction extends UndoableAction {
    ...
    _postChangeUpdates(highlight = false) {
        ...
        _postChangeUpdates(this._widgetEditor, this._widgetConnector, this._eventCenterService, [this._elementId], highlight, bCaptionManuallyChanged);
    }
    ...
    canUndo() {
        return !!this._oldParams &&
            !!this._oldConnections &&
            _datanodesExists(_collectUsedDatasources(this._oldConnections));
    }
...
class ClearWidgetConnectionsAction extends UndoableAction {
    ...
    _postChangeUpdates(highlight = false) {
        _postChangeUpdates(this._widgetEditor, this._widgetConnector, this._eventCenterService, this._elementIds, highlight);
    }
...
angular.module('modules.editor').service('EditorActionFactory', ['WidgetsPluginsHandlerGetter', 'WidgetContainerGetter', 'WidgetEditorGetter', 'WidgetConnectorGetter', 'EventCenterService', 'LayoutMgrGetter',
    function(widgetsPluginsHandlerGetter, widgetContainerGetter, widgetEditorGetter, widgetConnectorGetter, eventCenterService, layoutMgrGetter) {
        ...
        this.createSetGeometryAction = function _createSetGeometryAction(geometries, label) {
            ...
            return new UpdateGeometryAction(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, geometries, label || `Set widget${plural} position/size`, false);
        }
        ...
        this.createSetWidgetLeftAction = function _createSetWidgetLeftAction(elementIds, left, canMerge = false) {
            ...
            return UpdateGeometryAction.CreateSimpleUpdate(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, elementIds, { left: left + minLeftCst }, `Set widget${plural} X to ${left}`, false, canMerge ? Date.now() + UNDO_MOVE_MERGE_WINDO_MS : undefined);
        }
        ...
        this.createSetWidgetTopAction = function _createSetWidgetTopAction(elementIds, top, canMerge = false) {
            const plural = elementIds.length > 1 ? "s" : "";
            return UpdateGeometryAction.CreateSimpleUpdate(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, elementIds, { top: top + minTopCst }, `Set widget${plural} Y to ${top}`, false, canMerge ? Date.now() + UNDO_MOVE_MERGE_WINDO_MS : undefined);
        }
        ...
        this.createSetWidgetHeightAction = function _createSetWidgetHeightAction(elementIds, height, canMerge = false) {
            const plural = elementIds.length > 1 ? "s" : "";
            return UpdateGeometryAction.CreateSimpleUpdate(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, elementIds, { height }, `Set widget${plural} height to ${height}`, false, canMerge ? Date.now() + UNDO_MOVE_MERGE_WINDO_MS : undefined);
        }
        ...
        this.createSetWidgetWidthAction = function _createSetWidgetWidthAction(elementIds, width, canMerge = false) {
            const plural = elementIds.length > 1 ? "s" : "";
            return UpdateGeometryAction.CreateSimpleUpdate(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, elementIds, { width }, `Set widget${plural} width to ${width}`, false, canMerge ? Date.now() + UNDO_MOVE_MERGE_WINDO_MS : undefined);
        }
        ...
        this.createMoveDownAction = function _createMoveDownAction(elementIds, distance) {
            const plural = elementIds.length > 1 ? "s" : "";
            return UpdateGeometryAction.CreateSimpleUpdate(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, elementIds, { top: distance }, `Move widget${plural} down`, true, Date.now() + UNDO_MOVE_MERGE_WINDO_MS);
        }
        ...
        this.createMoveUpAction = function _createMoveUpAction(elementIds, distance) {
            const plural = elementIds.length > 1 ? "s" : "";
            return UpdateGeometryAction.CreateSimpleUpdate(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, elementIds, { top: -distance }, `Move widget${plural} up`, true, Date.now() + UNDO_MOVE_MERGE_WINDO_MS);
        }
        ...
        this.createMoveLeftAction = function _createMoveLeftAction(elementIds, distance) {
            const plural = elementIds.length > 1 ? "s" : "";
            return UpdateGeometryAction.CreateSimpleUpdate(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, elementIds, { left: -distance }, `Move widget${plural} left`, true, Date.now() + UNDO_MOVE_MERGE_WINDO_MS);
        }
        ...
        this.createMoveRightAction = function _createMoveRightAction(elementIds, distance) {
            const plural = elementIds.length > 1 ? "s" : "";
            return UpdateGeometryAction.CreateSimpleUpdate(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, elementIds, { left: distance }, `Move widget${plural} right`, true, Date.now() + UNDO_MOVE_MERGE_WINDO_MS);
        }
        ...
        this.createAlignedMoveDownAction = function _createAlignedMoveDownAction(elementIds) {
            ...
            const positions = _collectOtherWidgetsPotision(widgetEditor, elementIds).map(g => g.top).sort();
            const current = getElementLayoutPx(widgetEditor.widgetContainers.get(elementIds[elementIds.length - 1]).divModel).top;
            const target = Math.min(...positions.filter(p => p > current));
            const top = isFinite(target) ? target : current;

            const plural = elementIds.length > 1 ? "s" : "";
            return UpdateGeometryAction.CreateSimpleUpdate(widgetContainerGetter(), widgetEditor, eventCenterService, elementIds, { top }, `Aligned move widget${plural} down`);
        }
        ...
        this.createAlignedMoveUpAction = function _createAlignedMoveUpAction(elementIds) {
            ...
            const plural = elementIds.length > 1 ? "s" : "";
            return UpdateGeometryAction.CreateSimpleUpdate(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, elementIds, { top }, `Aligned move widget${plural} up`);
        }
        ...
        this.createAlignedMoveLeftAction = function _createAlignedMoveLeftAction(elementIds) {
            const widgetEditor = widgetEditorGetter();
            const positions = _collectOtherWidgetsPotision(widgetEditor, elementIds).map(g => g.left).sort();
            const current = getElementLayoutPx(widgetEditor.widgetContainers.get(elementIds[elementIds.length - 1]).divModel).left;
            const left = Math.max(0, Math.max(...positions.filter(p => p < current)));

            const plural = elementIds.length > 1 ? "s" : "";
            return UpdateGeometryAction.CreateSimpleUpdate(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, elementIds, { left }, `Aligned move widget${plural} left`);
        }
        ...
        this.createAlignedMoveRightAction = function _createAlignedMoveRightAction(elementIds) {
            ...
            const plural = elementIds.length > 1 ? "s" : "";
            return UpdateGeometryAction.CreateSimpleUpdate(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, elementIds, { left }, `Aligned move widget${plural} right`);
        }
        ...
        this.createAlignementTopAction = function _createAlignementTopAction(targetId, elementIds) {
            ...
            return UpdateGeometryAction.CreateSimpleUpdate(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, elementIds, { top }, `Align widgets top`);
        }
        ...
        this.createAlignementHorizontalAction = function _createAlignementHorizontalAction(targetId, elementIds) {
            ...
            return new UpdateGeometryAction(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, geometries, "Align widgets horizontal", false);
        }
        ...
        this.createAlignementBottomAction = function _createAlignementBottomAction(targetId, elementIds) {
            ...
            return new UpdateGeometryAction(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, geometries, "Align widgets bottom", false);
        }
        ...
        this.createAlignementLeftAction = function _createAlignementLeftAction(targetId, elementIds) {
            ...
            return UpdateGeometryAction.CreateSimpleUpdate(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, elementIds, { left }, `Align widgets left`);
        }
        ...
        this.createAlignementVerticalAction = function _createAlignementVerticalAction(targetId, elementIds) {
            ...
            return new UpdateGeometryAction(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, geometries, "Align widgets vertical", false);
        }
        ...
        this.createAlignementRightAction = function _createAlignementRightAction(targetId, elementIds) {
            ...
            return new UpdateGeometryAction(widgetContainerGetter(), widgetEditorGetter(), eventCenterService, geometries, "Align widgets right", false);
        }
        ...
        this.createCreateWidgetdAction = function _createCreateWidgetdAction(modelJsonId, containerDivOrId, layout) {
            ...
            if (widgetsPluginsHandler && widgetsPluginsHandler.widgetToolbarDefinitions) {
                ...
                if (def && def.title) {
                    return new CreateWidgetAction(widgetEditorGetter(), modelJsonId, `Add "${def.title}"`, containerDivOrId, layout);
                }
            }
        ...
        this.createDeleteWidgetsAction = function _createDeleteWidgetsAction(elementIds) {
            return new DeleteWidgetsAction(widgetEditorGetter(), widgetConnectorGetter(), widgetContainerGetter(), elementIds);
        }
        ...
        this.createDeleteAllWidgetsAction = function _createDeleteWidgetsAction() {
            return new DeleteWidgetsAction(widgetEditorGetter(), widgetConnectorGetter(), widgetContainerGetter(), [...widgetEditor.widgetContainers.keys()], "Delete all widgets");
        }
        ...
        this.createClearAllWidgetConnectionsAction = function _createClearAllWidgetConnectionsAction() {
            return new ClearWidgetConnectionsAction(widgetEditorGetter(), widgetConnectorGetter(), eventCenterService, [...widgetEditor.widgetContainers.keys()], "Clear all connections");
        }
         ...
         */
        this.createSetWidgetParametersAction = function _createSetWidgetParametersAction(elementId, newData, widgetConnection) {
            return new SetWidgetParametersAction(widgetEditorGetter(), widgetContainerGetter(), widgetConnectorGetter(), eventCenterService, elementId, newData, widgetConnection);
        }
