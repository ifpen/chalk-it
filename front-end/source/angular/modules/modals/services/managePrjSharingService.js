...
angular
    .module('modules')
    .service('ManagePrjSharingService', ['$rootScope', function ($rootScope) {
        ...
        self.shareProject = function (fileName, fileType) {
            ...
            if (_.isUndefined(fileType))
                $rootScope.sharedUserEmail.fileType = _translateExtension("xprjson");
            else
                $rootScope.sharedUserEmail.fileType = _translateExtension(fileType);
        ...
        function _initShareProject(fileName, fileType, callback) {
            if ($rootScope.sharedUserEmail.fileType === "project") {
                if (_.isUndefined($rootScope.currentInfoProject))
                    $rootScope.currentInfoProject = {};
        ...
        self.updateTypedEmail = function () {
            if (!_.isNull($rootScope.sharedUserEmail.selected) && $rootScope.sharedUserEmail.selected.id != 0)
                ...
                $rootScope.sharedUserEmail.typed = "";
        };
