...
.component('xdashJsonEditor', {
    ...
    controller: ['$element', '$scope', class GraphicalPropertiesEditor {
...
.component('xdashWidgetConnectionsEditor', {
    ...
    controller: ['$window', '$timeout', 'UiNotifications', class WidgetConnectionsEditor {
        ...
        $onChanges(changes) {
            const connectionChanged = changes.widgetConnections && changes.widgetConnections.currentValue !== changes.widgetConnections.previousValue;
        ...
        _grabSliderDescriptions(connections, params) {
            return connections && connections.widgetObjEdit ? [... connections.widgetObjEdit.getActuatorDescriptions(params)] : [];
        }
        ...
        _updateContent(newConnections) {
            this.dataNodes = []
            if(this.$window.datanodesManager) { // Probably not initialized before angular. TODO should be a service
            ...
            for (const actuatorName of this.sliderNames) {
                ...
                if (slider)  {
                    ...
                    if(ds && desc.dsFilter(ds)) {
                        ...
                        slider.dataFields = this.selectionCombos[actuatorName].fieldsCombos.map(_ => _.selectedOpt.key).filter(_ => _ !== undefined);
                    } else {
                        if(slider.dataNode !== "None") {
                            console.log("DataNode connection is removed because '", slider.dataNode, "' dataNode does not exist!");
                        }
                        slider = undefined;
                    }
                } 
        ...
        _buildOptions(valueDef) {
            ...
            if(value === null || typeof value !== "object") { // 'object' does include arrays
                return null;
            }
            ...
            if(size <= WidgetConnectionsEditor.MAX_VAL_PER_COMBO) {
                ...
                if(increment > size) {
                    ...
                    console.warn(`COMBO_SCALE_FACTOR=${WidgetConnectionsEditor.COMBO_SCALE_FACTOR} too big relative to MAX_VAL_PER_COMBO=${WidgetConnectionsEditor.MAX_VAL_PER_COMBO}`);
        ...
        onDatafieldChange(sliderName, index) {
            ...
            slider.dataFields = this.selectionCombos[sliderName].fieldsCombos.map(_ => _.selectedOpt.key).filter(_ => _ !== undefined);
            this._notifyChanges();
        }
    }],
    template: `
    ...
    <div class="dataconnection__row">
        <div class="dataconnection__col">
            ...
            <select id="DS{{sliderName}}" ng-model="$ctrl.currentConnections.sliders[sliderName].dataNode" ng-change="$ctrl.onDatasourceChange(sliderName)">
                <option>None</option>
                <option ng-repeat="ds in $ctrl.dataNodes | filter: $ctrl.sliderDescriptions[sliderName].dsFilter" ng-class="{'validated' : ds.validated[sliderName]}">{{ds.name}}</option>
            </select>
        </div>
...
.controller('DashboardRightSidePanelController', ['$scope', '$rootScope', '$state', '$q', 'UndoManagerService', 'EditorActionFactory', 'WidgetEditorGetter',
    function($scope, $rootScope, $state, $q, undoManagerService, editorActionFactory, widgetEditorGetter) {
        ...
        vm.savePanel = function _savePanel() {
            ...
            if (!angular.equals(newData, vm.originalWidgetParams) || !angular.equals(_normalizeWidgetConnector(newConnections), _normalizeWidgetConnector(vm.originalWidgetConnection))) {
        ...
        vm.showPopup = function _showPopup(title, textBtnYes = "Yes", textBtnNo = "No") { // Promise<boolean>
            return new $q(resolve => vm.popup = {title, textBtnYes, textBtnNo, resolve}).finally(() => vm.popup = null);
        }

        vm.requestResetDataConnection = function _requestResetDataConnection() {
            return vm.showPopup("Are you sure you want to erase the DataNode Connections ?", "Yes", "Abandon")
                .then(ok => {
                    if(ok) {
                        vm.resetPanel();
                    }
        ...
        vm.requestResetGraphicalProperties = function _requestResetGraphicalProperties() {
            return vm.showPopup("Are you sure you want to reset the Graphical Properties ?", "Yes", "Abandon")
                .then(ok => {
                    if(ok) {
        ...
        vm.requestSavePanel = function _requestSavePanel() {
            return vm.showPopup("Do you want to save your current changes ?")
                .then(ok => {
                    if(ok) {
                        vm.savePanel();
                    }
