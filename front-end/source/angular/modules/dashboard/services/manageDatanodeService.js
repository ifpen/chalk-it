...
angular
    ...
    .service('ManageDatanodeService', ['$rootScope', function($rootScope) {
        ...
        self.applyDatanodeFilter = function() {
            ...
            setTimeout(function() {
                for (let i = 0; i < $rootScope.alldatanodes.length; i++) {
                    ...
                    if (!($rootScope.alldatanodes[i]._serializedData.settings.name.toLowerCase().indexOf(tmpStr.toLowerCase()) >= 0)) {
                        $rootScope.alldatanodes[i].hide = true;
                    } else {
                        if ($rootScope.filtredList.length === 0)
                            $rootScope.alldatanodes[i].hide = false;
