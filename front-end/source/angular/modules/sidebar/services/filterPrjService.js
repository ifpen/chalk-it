    ...
    self.ProjectsFilter = function (tmpStr, vm) {
      setTimeout(function () {
        ...
        for (let j = 0; j <= vm.nbCat - 1; j++) {
          ...
          $rootScope.pagination.totalPages = Math.ceil(totalShowedData / vm.perPage);
      ...
      setTimeout(function () {
        ...
        for (let j = 0; j <= vm.nbCat - 1; j++) {
          ...
          if (_.isUndefined(element)) {
            //when called after a filter value
            element = {};
            element.className = '';
          }

          if (element.className.includes('active')) {
            //add tag on current filter
            currentFiles = angular.copy(vm.allFilesFiltred[j]); //start from current filter
            TagList[0] = Str;
          } else {
            //remove current selected tag and re-aply past tag filters
            ...
            for (let i in TagList) {
              ...
              for (var tt = 0; tt < currentFiles.FileList.length; tt++) {
                ...
                if (currentFiles.FileList[tt].Tags) {
                  for (var tg = 0; tg < currentFiles.FileList[tt].Tags.length; tg++) {
                    if (currentFiles.FileList[tt].Tags[tg].toLowerCase().indexOf(tmpStr.toLowerCase()) >= 0) {
                      hide = false;
                    }
                  }
                }
                ...
                if (!hide) {
                  totalShowedData++;
                  tempFile.push(currentFiles.FileList[tt]);
                }
              }
