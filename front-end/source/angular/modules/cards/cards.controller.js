        ...
        return {
            ...
            link: function(scope, element, attrs) {
                element.bind('click', function() {
                    ...
                    if (!_.isUndefined($("#search-input")[0]))
                        $("#search-input")[0].value = "";
                    if (!_.isUndefined($("#search-input-sidebar")[0]))
                        $("#search-input-sidebar")[0].value = "";
                });
            }
        };
    })
    .controller("CardsController", ["$scope", "$rootScope", "$state", "$http", "ApisFactory", "cfpLoadingBar", "FilterPrjService", "ManagePrjService", "ManagePageSharingService", "ManagePrjSharingService",
        function($scope, $rootScope, $state, $http, ApisFactory, cfpLoadingBar, FilterPrjService, ManagePrjService, ManagePgSharingService, ManagePrjSharingService) {
            ...
            $rootScope.updateView = function() {
                ...
                if (_.isNull(action))
                    ...
                    .then($scope.findFilesByFilters(fileType));
                else
                    $state.go("modules.cards.layout", { action: action })
                    .then($scope.findFilesByFilters(fileType));
            };
            ...
            function _manageGrpFile() {
                ...
                if (!_.isUndefined($scope.allFilesWithNoGrp[0])) { //when no project exist
                    for (let i = 0; i < $scope.allFilesWithNoGrp[0].FileList.length; i++) { //group only for [0] ==> project
            ...
            function _initFiles(file) {
                ...
                if ($scope.projectVue === "gallery") { //gallery of template view
            ...
            function _responseCallback(result, msg2, type) {
                ...
                if ($scope.projectVue === "all") { //my projects view
                    _manageGrpFile();
                }
                _initPagination();
            ...
            $scope.resetFilters = function() { //AEF: reset filters (keywords value and tags list)
            ...
            /*---------- sortFiles:Sort btn ----------------*/
            $scope.sortFiles = function() { //AEF: sort and reverse cards view
                if (!_.isUndefined($scope.allFilesWithNoGrp[0])) { //when no project exist
            ...
            $scope.showPageLink = function(pageName) {
                ...
                var FileMngrInst = new FileMngrFct();
                FileMngrInst.GetThumbnailURL("page", pageName + ".html", function(msg1, msg2) {
                    if (msg1) {
            ...
            $scope.toggleTagDisplay = function(index, origin, grpIndex) {
                if (origin === "group") {
                    ...
                    //
                    if (index == $rootScope.displayedIndexG[grpIndex])
                        $rootScope.displayedIndexG[grpIndex] = -1;
                    else
                        $rootScope.displayedIndexG[grpIndex] = index;
                } else {
                    //reset
                    for (let i = 0; i < $rootScope.displayedIndexG.length; i++)
                        $rootScope.displayedIndexG[i] = -1;
                    //
                    if (index == $rootScope.displayedIndex)
                        $rootScope.displayedIndex = -1;
                    else
                        $rootScope.displayedIndex = index;
                }
            };
