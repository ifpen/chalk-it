...
var htmlExport = (function() {
    ...
    function emptyDashboardDiv() {
        ...
        dashDiv.innerHTML = '<div id="DropperDroitec" class="dropperR" style="position:relative;z-index:0;border-radius:10px" ng-app="xCLOUD"></div>';
        return dashDiv;
    }
    ...
    function copyHeadContent() {
        ...
        for (var t = 0; t < exportHeaderCss.length; t++) {
            ...
            // }
            scriptHeaderList.push('<link rel="stylesheet" href="' + _urlBaseForExport + '' + exportHeaderCss[t] + '"></link>');
        }
        for (t = 0; t < exportHeaderJs.length; t++) {
            scriptHeaderList.push('<script charset="UTF-8" src="' + xDashConfig.urlBaseForExport + exportHeaderJs[t] + '"></script>');
        }
    ...
    function copyBodyContent(xdashPrj) {
        ...
        bodyText3.push(
            'reconstructFoundations.loadXprjson(jsonContent);',
            '});',
            '</script>',
            ''
        );
        ...
        bodyText = bodyText0.join("\n") + bodyText1.join("\n") + bodyTextJson + bodyText2.join("\n") + bodyText3.join("\n") + bodyText4.join("\n");
    ...
    function previewDashboard(xprjson, projectName, bNoExportModal) {
        ...
        if ((htmlExport.checkExportOptions) || bNoExportModal)
            previewDashboardCallback(param);
    }
    ...
    function previewDashboardCallback(param) {
        ...
        if (_.isUndefined(projectName))
            dashboardName = $('#projectName')[0].value;
        else
