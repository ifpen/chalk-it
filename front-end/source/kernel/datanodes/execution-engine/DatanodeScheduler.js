...
function DatanodeScheduler(datanodesDependency, startNodes, initiatorNode, callOrigin) {
    ...
    function isCalledFromOrchestrator(nodeName) {
        if (_.contains(startNodes, nodeName)) { // MBG 10/05/2019 : more general condition
            switch (callOrigin) {
                case "triggerButton":
                case "vignette":
                case "setValue":
        ...
        operationsToExecute.forEach(function (op) {
            if (operationsToExecute.size == 0) {
                if (!offSchedLogUser && !xDashConfig.disableSchedulerLog)
                    console.log(op + ' is already treated');
                return;
            }
                ...
                case "Ready":
                    if (datanodesManager.foundDatanode(op)) {
                        ...
                        if (bAllPredExecuted) {
                            ...
                            datanodesManager.getDataNodeByName(op).updateNow(bCalledFromOrchestrator, bForceAutoStart, bAllPredExecuted);
                        } else {
                            operationsToExecute.delete(op);
                            operationsNotReady.add(op);
                            if (callOrigin == "refresh" || callOrigin == "triggerButton") {
                                swal('Update is not available', 'Predecessors: ' + notExecutedPredecessors(op) + ' of operation "' + op + '" need to be executed first.', "info");
                                datanodesManager.getDataNodeByName(op).completeExecution("NOP");
                            } else {
                                if (callOrigin == "edit" || callOrigin == "unidentified") { //a la creation ou a l'edition, quand le user declare une datanode qui n'extste pas, il faut passer cette etape pour le voir
    ...
    function operationCompleted(dsName, status) {
        ...
        switch (status) {
            ...
            case "NOP": // NOP is for handling explicit_trig case when called from scheduler (behave like in error)
                {
                    if (datanodesManager.getDataNodeByName(dsName).status() === 'OK') { // ds has already ok status, even it is not explicitly triggered, their successors can be computed
                        operationsTerminated.add(dsName);
                        operationsToWaitFor.delete(dsName);
                        if (!offSchedLogUser && !xDashConfig.disableSchedulerLog) {
                            console.log('operation ' + dsName + ' completed with status ' + status);
                        ...
                        break;
                    } else if (datanodesManager.getDataNodeByName(dsName).status() === 'None' || datanodesManager.getDataNodeByName(dsName).status() === 'Error') {
                        operationsBlacklist.add(dsName);
                        operationsToWaitFor.delete(dsName);
                        dsNameDescendants = datanodesDependency.getDescendants([dsName]); //AEF: fix bug param of getDescendants must be an array
                        dsNameDescendants.forEach(function (elem) {
                    ...
                    } else // not sure this case exists
                        console.log("datanode with explicitTrig ended with status ", datanodesManager.getDataNodeByName(dsName).status());
                    break;
                }
            default:
