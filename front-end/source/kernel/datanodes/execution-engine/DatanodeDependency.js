...
function DatanodeDependency() {
    ...
    function getDisconnectedGraphs(startNodes, disconnectedGraphs) {
        ...
        if (startNodes.length == 1 && disconnectedGraphs.length == 0) { // If one starnode, put the only one connected graph into disconnectedGraphs
            ...
            for (let i = 0; i < startNodesDesc.length; i++) {
                ...
                if (disconnectedGraphs.length == 0) { //At the begining
                    disconnectedGraphs.push(sameGraph); //put the first connected graph in disconnectedGraphs list
                } else {
                    for (let k in disconnectedGraphs) { //verify if the connected graph is already added to disconnectedGraphs list
                    ...
                    if (!bIntersection) { //if not added, add the new connected graph to the list
        ...
        disconnectedGraphs.some(function (disconGraph) {
            if (disconGraph.has(node)) {
                if (!offSchedLogUser && !xDashConfig.disableSchedulerLog)
                    console.log("found ", node, " in", disconGraph);
                graph = disconGraph;
            }
        });
        return graph;
    ...
    function setExtraStartNodes(dsName, callOrigin) {
        if (!_.isUndefined(extraStartNodes[dsName])) {
            xdashNotifications.manageNotification("info", dsName, dsName + " is called many consecutif times while scheduler is in progress");
        }
