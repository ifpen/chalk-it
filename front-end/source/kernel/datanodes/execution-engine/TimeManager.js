...
function TimeManager() {
    ...
    function isRegisteredDatanode(dsName) {
        if (!_.isUndefined(periodsMap[dsName]))
            return true;
        else
            return false;
    }
    ...
    function registerDatanode(sampleTime, dsName, callOriginArg) {
        ...
        if (startNodes.indexOf(dsName) == -1) { //dsName does not already exist in startNodes
    ...
    function unregisterDatanode(dsName) {
        ...
        if (index != -1) { //dsName exist in startNodes
            startNodes.splice(index, 1);
        } else {
            if (!offSchedLogUser && !xDashConfig.disableSchedulerLog)
                console.log("error in datanode " + dsName);
            return;
        }
        // optional: reset tick
        currentTick = 0;
        if (Object.keys(periodsMap).length == 0) { // clear timer when all periodic datanodes are deleted
