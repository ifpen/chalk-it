...
function GraphVisu(datanodesDependency) {
    ...
    function showDepGraph(selectedNodeName) {
        ...
        for (var j = 0; j < nodeNames.length; j++) {
            nodeEdges = Array.from(
                datanodesDependency.dependencyStructure[nodeNames[j]]
            );
    ...
    function startNetwork(data, selectedNodeName) {
        ...
        var networkCanvas = document
            .getElementById("dependencyGraphBody")
    ...
    function selectNodebyName(nodeName) {
        ...
        if (typeof nodeName === "string") {
            nodesGlobal.update([
                { id: nodesLabelsIds[nodeName], color: { background: "orange" } },
            ]);
    ...
    function selectNodeFromTagList(nodeType) {
        ...
        if (tagElementType.classList.contains("active")) {
            ...
            for (let k = 0; k < foundElements.length; k++) {
                highlightNodeByType.splice(
                    highlightNodeByType.indexOf(foundElements[k]),
                    1
                );
            }
        } else {
    ...
    function selectConnectedWithWidget(choice) {
        ...
        if (tagElement.classList.contains("active")) {
            ...
            for (let i = 0; i < highlightNodeByWidgetConnection.length; i++) {
                if (
                    highlightNodeByWidgetConnection[i]["color"]["border"] === "green" &&
                    choice === "connectedtowidget"
                ) {
                ...
                if (
                    highlightNodeByWidgetConnection[i]["color"]["border"] === "red" &&
                    choice === "notconnectedtowidget"
                ) {
                    highlightNodeByWidgetConnection[i]["color"] = { border: "#478dec" };
                    highlightNodeByWidgetConnection[i]["borderWidth"] = 1;
                    foundElements.push(highlightNodeByWidgetConnection[i]);
            ...
            for (let k = 0; k < foundElements.length; k++) {
                highlightNodeByWidgetConnection.splice(
                    highlightNodeByWidgetConnection.indexOf(foundElements[k]),
                    1
                );
            }
        } else {
            ...
            for (let j = 0; j < nodesTmp.length; j++) {
                if (
                    nodesTmp[j].widgetConnection === true &&
                    choice === "connectedtowidget"
                ) {
                ...
                if (
                    nodesTmp[j].widgetConnection === false &&
                    choice === "notconnectedtowidget"
                ) {
                    highlightNodeByWidgetConnection.push({
                        id: nodesLabelsIds[nodesTmp[j].label],
            ...
            if (countFoundedNode === 1) {
                focusOnNode(
                    highlightNodeByWidgetConnection[
                        highlightNodeByWidgetConnection.length - 1
                    ]["id"].toString()
                );
            } else {
                fitAnimated();
