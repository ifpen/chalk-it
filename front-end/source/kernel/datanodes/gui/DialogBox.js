...
function DialogBox(contentElement, title, okTitle, cancelTitle, okCallback, cancelCallback) {
    ...
    function cancelFunction() {
        if (_.isFunction(cancelCallback)) {
            cancelCallback();
        } else if (!_.isUndefined(cancelCallback)) { // this case is not a cancelCallback but a parameter needed for okCallback
    ...
    if (cancelTitle) {
        $('<input id="dialog-cancel" type="button" class="btn btn-rounded-fill cancel"  value="' + cancelTitle + '"></input>').appendTo(footer).click(cancelFunction);
    ...
    function okFunction() {
        ...
        if (_.isFunction(okCallback)) {
            if (param)
                hold = okCallback(param);
            else if (OkClbkParam) //AEF used by DialogBoxForHtmlExport
    ...
    if (okTitle) {
        $('<input id="dialog-ok" type="button" class="btn btn-rounded-fill primary" value="' + okTitle + '"></input>').appendTo(footer).click(okFunction);
    }
