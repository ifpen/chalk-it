...
var datanodesManager = (function () {
    ...
    var datanodesListModel = new DatanodesListModel(
        datanodePlugins,
        freeboardUI,
        datanodesDependency,
        timeManager
    ...
    function deleteDn(viewModel) {
        ...
        if (datanodesDependency.hasSuccessors(viewModel.name())) {
            let successors = Array.from(
                datanodesDependency.getSuccessors(viewModel.name())
            );
        ...
        datanodesDependency.updateDisconnectedGraphsList(
            viewModel.name(),
            "delete"
        );
        ...
        if (!offSchedLogUser && !xDashConfig.disableSchedulerLog)
            console.log(
                "All disconnected Graphs after delete: ",
                datanodesDependency.getAllDisconnectedGraphs()
            );
        //
    }
        ...
        for (var prop in widgetConnector.widgetsConnection) {
            for (var i in widgetConnector.widgetsConnection[prop].sliders) {
                if (widgetConnector.widgetsConnection[prop].sliders[i].name != "None") {
                    if (
                        widgetConnector.widgetsConnection[prop].sliders[i].dataNode ===
                        DSname
                    ) {
            ...
            swal(
                {
                    title: "Are you sure?",
                    text:
                        "This dataNode will be deleted and connections with widget(s) '" +
                        wdList.join("\n") +
                        "' may be removed!",
                    type: "warning",
    ...
    function settingsSavedCallback(viewModel, options, newSettings) {
        if (options.operation == "add") {
            ...
            if (options.type == "datanode") {
                ...
                newViewModel = new DatanodeModel(
                    datanodesListModel,
                    datanodePlugins,
                    datanodesDependency,
                    timeManager
                ...
                let iconPath = "source/assets/icon/datanodes/" + newSettings.iconType;
                newViewModel.iconType(iconPath);
                ...
                if (!offSchedLogUser && !xDashConfig.disableSchedulerLog)
                    console.log(
                        "All disconnected Graphs after add: ",
                        datanodesDependency.getAllDisconnectedGraphs()
                    );
                //
                ...
                //AEF
                if (
                    _.isUndefined(newSettings.settings.sampleTime) ||
                    newSettings.settings.sampleTime == 0
                ) {
                    newViewModel.schedulerStart(undefined, undefined, "unidentified");
                } else {
                    newViewModel.sampleTime(newSettings.settings.sampleTime);
                    timeManager.registerDatanode(
                        newViewModel.sampleTime(),
                        newViewModel.name(),
                        "unidentified"
                    );
                }
            ...
            if (options.type == "datanode") {
                if (viewModel.isSettingNameChanged(newSettings.settings.name)) {
                    ...
                    //
                    datanodesListModel.renameDatanodeData(
                        viewModel.name(),
                        newSettings.settings.name
                    );
                }
                datanodesDependency.renameNode(
                    viewModel.name(),
                    newSettings.settings.name
                );
                ...
                if (!_.isUndefined(newSettings.settings.sampleTime)) {
                    if (
                        viewModel.isSettingSampleTimeChanged(
                            newSettings.settings.sampleTime
                        )
                    ) {
            ...
            if (options.type == "datanode") {
                ...
                if (!offSchedLogUser && !xDashConfig.disableSchedulerLog)
                    console.log(
                        "All disconnected Graphs after rename: ",
                        datanodesDependency.getAllDisconnectedGraphs()
                    ); //To optimize after
                //
                if (!_.isUndefined(newSettings.settings.sampleTime)) {
    ...
    return {
        ...
        showDepGraph: function (name) {
            ...
            let scopeDepGraph = angular
                .element(document.getElementById("dependency__graph--container"))
                .scope();
        ...
        RemoveUnusedDatanodes: function () {
            ...
            console.log("zombie:", zombieDatanodeList);
                ...
                function () {
                    for (var i = 0; i < zombieDatanodeList.length; i++) {
                        if ($("#data-checkbox-" + i).is(":checked")) {
                            ...
                            deleteDn(
                                datanodesManager.getDataNodeByName(zombieDatanodeList[i])
                            );
                            bFound = true;
                        }
                    }
        ...
        settingsSavedCallback: function (
            viewModel,
            options,
            newSettings,
            selectedType
