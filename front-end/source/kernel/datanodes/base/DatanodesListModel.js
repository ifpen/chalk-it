...
function DatanodesListModel(datanodePlugins, freeboardUI, datanodesDependency, timeManager) {
    ...
    this.deserialize = function (object, bClear, finishedCallback) //ABK
    {
        ...
        function finishLoad() {
            ...
            for (let i = 0; i < object.datanodes.length; i++) { //compatibility
                ...
                if (diff != 0) {
                    for (var prop in datanodesDependency.dependencyStructure) {
                        ...
                        if (!bFound) {
                            ...
                            swal("Missed dataNodes!", "'" + prop + "' is referenced in formula of '" + successors + "' but doesn't exist in list of datanodes", "error");
                            diff--;
                        }
                        if (diff == 0) {
    ...
    function displayDuplicateDataList() {
        var contentElement = xdsjson.getDuplicateDataList(storeData, "Please check data to be loaded after choosing rename or overwite option");
        ...
        new DialogBoxForDuplicateData(contentElement, "List of duplicate data name", "Load", "Close", function () {
            ...
            for (i = 0; i < storeData.length; i++) {
                if ($('#data-checkbox-' + i).is(':checked')) {
                    storeData[i].name = $('#data-check-' + i)[0].value;
                    if ($('#data-rename-' + i).is(':checked')) { //add new one
    ...
    this.launchGlobalFirstUpdate = function (datanode) {
        ...
        if (sourceNodes.length != 0) { //AEF: only if no-periodic datanodes exist as a startnodes
