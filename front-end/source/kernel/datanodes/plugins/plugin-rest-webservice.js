...
(function () {
  var jsonDatanode = function (settings, updateCallback, statusCallback, bodyType, notificationCallback) {
      ...
      function WebserviceRequestSettings() {
          ...
          if (currentSettings.use_xproxy) {
              if (_.isUndefined(currentSettings.proxyHash))
                  currentSettings.proxyHash = "";
              if (_.isUndefined(currentSettings.proxySig))
      ...
      function webserviceRequest(requestURL, reqDataType, data) {
          ...
          if (jqXHR_hash && jqXHR_hash.readyState != 4) {
              jqXHR_hash.done(function () { //wait for hash response 
      ...
      function callWebservice(requestURL, reqDataType, body) {
          ...
          jqXHR = $.ajax({
                          ...
                          if (names.length === 1 && names[0] === "d") { 
                              ...
                              if (data.Success) {
                                  ...
                                  if (!_.isUndefined(data.Message)) {
                                      ErrorMessage = "xProxy returned error at dataNode \"" + currentSettings.name + "\" : \"" + data.Message + "\"";
              ...
              error: function (xhr, status, err) {
                  ...
                  if (!_.isUndefined(xhr.responseText)) {
                      ...
                      if (index1 != -1 && index2 != -1)
                          errorMessage = xhr.responseText.slice(index1, index2);
                      else
              ...
              complete: function (xhr, status) { //AEF
          ...
          interval = setInterval(function () {
              ...
              if (isOnLine) {
                  ...
                  if (!_.isUndefined(jqXHR)) {
                      ...
                      notificationCallback("error", currentSettings.name, "Internet is disconnected. Request of '" + currentSettings.name + "' wil be aborted");
                      jqXHR.abort();
                  }
              }
          }, 1000);
  ...
  function preFillHeaders() {
      if (!_.isUndefined($('input#table-row-value-headers0')[0])) { //FMI_web-service_from_JSON_editor
  ...
  datanodesManager.loadDatanodePlugin({
      ...
      },
      {
          ...
          description: "xProxy allows your browser to call cross-domain REST APIs (which solves browser CORS policy issues). Keep this parameter to true, unless your distant REST API allows your browser to be called in a cross-origin context.",
          type: "boolean",
          default_value: true
      },
      {
      ...
      },
      {
          ...
          description: "Execute web-service only if triggered explicitly : no execution when predecessor dataNodes are modified.",
          type: "boolean",
          default_value: false
      },
      {
      ...
      },
      {
          ...
          description1: "Insert the body of the request. Normally only used if method is POST. If the method is GET, the content of urlAppend object will be appended to the URL. The content of headersFromDataNodeWS will be injected to headers",
      ...
      newInstance: function (settings, newInstanceCallback, updateCallback, statusCallback, notificationCallback) {
          newInstanceCallback(new jsonDatanode(settings, updateCallback, statusCallback, "calculated", notificationCallback));
          if (error) //ABK
              return false;
          else
              return true;
  ...
  if (xDashConfig.jsonEditorDatanodes) {
      datanodesManager.loadDatanodePlugin({
          ...
          },
          {
              name: "use_xproxy",
              display_name: "Use xProxy",
              description: "If that also fails due to CORS policy, you can use xProxy, which can solve many connection problems to APIs.",
              type: "boolean",
              default_value: true
          },
          {
          ...
          },
          {
              ...
              display_name: "Body",
              type: "json",
              description: "The body of the request. Normally only used if method is POST. If the method is GET, the content of \"urlAppend\" object it will be appended to the URL."
          },
          {
              name: "headers",
              display_name: "Headers",
