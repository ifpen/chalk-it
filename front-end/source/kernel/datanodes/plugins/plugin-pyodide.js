// TODO : handle globalScope lifecycle : delete when dataNode instance is deleted

(function () {
    var error = false;

    function isEmbedded() {
        return angular.element(document.body).injector().invoke(['$rootScope', function ($rootScope) {
    ...
    function displayPythonEditor(value, newSettings, settingDef, callback) {
        ...
        var exampleText = '#example: create 2 datanodes py_a a,d py_b then use them as follow:\n' +
        ...
        var codeWindowHeader = $('<div class="cancel__box__top">' +
            ...
            ' <br>Press <code><span class="cm-keyword">ctrl-space</span></code> to activate autocompletion.</div>' +
            '</div>');
        ...
        if (bFoundConnection) {
            var widgetSelect = '<div style="display: table-cell; padding: 10px">' +
            ...
            var widgetButton = $('<button id="dialog-widget" class="btn btn-rounded-fill primary" js-reset-toggle="">Show widget</button>').click(function () {
                if ($("#dialog-widget").text() == "Show widget") {
                    ...
                    _.each(prop, (instanceId) => {
                        widgetsPreviewDivs = widgetsPreviewDivs +
                            '<div id="widget-preview-zone-' + instanceId + '" ></div>';
                    });
                    var codeWidgetWindowHtml = '<div id="code-widget-window-js"  style="width: 49%;float: right;z-index:5000;overflow:auto;height:100%">' +
                    ...
                    var buildPreviewZone = function (instanceId) {
                        ...
                        var height = Math.min($("#" + instanceId).height(), $("#code-widget-window-js").height() - 40 -
                            $('#widgets-selection-header').height());
                        ...
                        var wd = widgetsPluginsHandler.copyWidget("widget-preview-zone-" + instanceId, modelJsonIdStr,
                    ...
                    _.each(prop, (instanceId) => { buildPreviewZone(instanceId); });
            ...
            var applyButton = $('<button id="apply-widget" class="btn btn-rounded-fill primary" js-reset-toggle="">Apply</button>').click(function () {
        ...
        var closeButton = $('<button id="dialog-cancel" class="btn btn-rounded-fill primary" js-reset-toggle="">Close </button>').click(function () {
            if (callback) {
    ...
    datanodesManager.loadDatanodePlugin({
        ...
        },
        {
            ...
            description: "Execute Python only if triggered explicitly : no execution when predecessor dataNodes are modified.",
            type: "boolean",
            default_value: false
        },
        {
        ...
        newInstance: function (settings, newInstanceCallback, updateCallback, statusCallback, notificationCallback) {
            if (!newInstanceCallback(new pyodideFormulaPlugin(settings, updateCallback, statusCallback, notificationCallback)))
                if (error)
                    return false;
                else
                    return true;
    ...
    var pyodideFormulaPlugin = function (settings, updateCallback, statusCallback, notificationCallback) {
        ...
        self.runScriptSettings = function () {
            ...
            if ((!_.isEmpty(currentSettings.args))) {
                ...
                script = currentSettings.content;
            ...
            if (!_.isEmpty(dataNodes)) {
                ...
                _.each(_.keys(dataNodes), (dnName) => {
                    if (datanodesManager.getDataNodeByName(dnName).type() == 'Python_pyodide_plugin') {
                        if (dnName != dataNodeName)
                            stdin = stdin + 'dataNodes["' + dnName + '"] = globalScope["' + dnName + '"]\n';
                    } else {
                        stdin = stdin + 'dataNodes["' + dnName + '"] = json.loads(js.JSON.stringify(js.datanodesManager.getDataNodeByName("' + dnName + '").latestData()))\n';
                    }
                });
            } else {
                stdin = stdin + `dataNodes = {};\n`;
...
globalScope['` + dataNodeName + `']=result
        ...
        self.runScript = async function (script, args) {
            ...
            var success = function (output) {
                ...
                if (!_.isUndefined(output)) {
                    if (output.constructor.name == 'PyProxyClass') // I think it's deadcode, but worth te be kept
                        data = output.toJs();
                    else
                        data = JSON.parseMore(output);
                }
            ...
            var error = function (status) {
                ...
                if (status === "abort") { //it's an Abort request
            ...
            const isDefaultLibs = (isStandardLibs && isMicropipLibs) ||
