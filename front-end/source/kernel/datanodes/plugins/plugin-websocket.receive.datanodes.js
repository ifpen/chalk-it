 ...
 */
(function() {
  var Websocket_receive_Plugin = function(settings, updateCallback, statusCallback, notificationCallback, statusForSchedulerCallback) {
      var self = this;
      var wsUri = "";
      ...
      var onMessage = function(event) {
          ...
          if (bFirstTime) { //in init state
              bFirstTime = false;
              if (debug) console.info("WebSocket received ", event.data);
              if ((typeof event.data === 'string') || (event.data instanceof String)) {
                  new_data = event.data;
              } else
                  new_data = processedData(event.data, currentSettings.decoding_options);
      ...
      self.updateNow = function(bCalledFromOrchestrator, bForceAutoStart) {
          ...
          if (bForceAutoStart) { // when refresh change autostart in setting (needed because behaviour it similar to periodic datanodeS)
          ...
          if (bFirstTime) {
              ...
              if ((typeof last_received_data === 'string') || (last_received_data instanceof String)) {
                  ...
                  new_data = processedData(last_received_data, currentSettings.decoding_options);
              // update new data to MyViz widgets
              statusCallback('OK');
              updateCallback(new_data);
              if (debug) console.info("Data processed ", new_data);
      ...
      function processedData(arrayBuffer, decoding_options) {
          if (decoding_options.toUpperCase() === "JSON") { //=== "JSON"
              try {
  ...
  datanodesManager.loadDatanodePlugin({
      ...
      settings: [{
          ...
          },
          {
              ...
              description: 'Start websocket receive automatically after dashboard play begins or after creation or modification.',
              type: "boolean",
              default_value: true
          },
          {
      ...
      newInstance: function(settings, newInstanceCallback, updateCallback, statusCallback, notificationCallback, statusForSchedulerCallback) {
          newInstanceCallback(new Websocket_receive_Plugin(settings, updateCallback, statusCallback, notificationCallback, statusForSchedulerCallback));
          if (error)
              return false;
          else
              return true;
