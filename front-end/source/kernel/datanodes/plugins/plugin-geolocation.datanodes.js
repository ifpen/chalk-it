...
if (!(xDashConfig.xDashBasicVersion == "true")) {
    (function() {
        var Geolocation_Plugin = function(settings, updateCallback, statusCallback, notificationCallback, statusForSchedulerCallback) {
            ...
            function showLocation(pos) { // it is triggered every coords change
                location = {
            ...
            function errorHandler(err) {
                var text;
                if (err.code == 1) { //retry with IP geolocation
        ...
        datanodesManager.loadDatanodePlugin({
            ...
            newInstance: function(settings, newInstanceCallback, updateCallback, statusCallback, notificationCallback, statusForSchedulerCallback) {
                newInstanceCallback(new Geolocation_Plugin(settings, updateCallback, statusCallback, notificationCallback, statusForSchedulerCallback));
                if (errorState)
                    return false;
                else
                    return true;
