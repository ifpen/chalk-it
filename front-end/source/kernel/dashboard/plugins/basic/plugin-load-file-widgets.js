...
function loadFileWidgetsPluginClass() {
	this.loadFileWidget = function (idDivContainer, idWidget, idInstance, bInteractive) {
		...
		function _processTextFile(fileExtension, fileData) {
			...
			return extensions.includes(fileExtension) ? JSON.parse(JSON.stringify(eval('(' + fileData + ')')), null, 2) : fileData;
		}
		...
		this.render = function () {
			...
			if (modelsParameters[idInstance].displayLabel) {
				...
				divContent +=
					'<div id="load-file-label' + idWidget + '" class="label-wrapper" style="' +
					this.labelFontSize() +
					this.labelColor() +
          ...
          <path d="M20.992 19.712c-0.192-0.192-0.576-0.192-0.768 0l-2.048 2.048c-0.192 0.192-0.192 0.576 0 0.768s0.576 0.192 0.768 0l1.088-1.088v3.648c0 0.32 0.256 0.576 0.576 0.576s0.512-0.256 0.512-0.576v-3.648l1.088 1.088c0.192 0.192 0.576 0.192 0.768 0s0.192-0.576 0-0.768l-1.984-2.048z"></path>
			...
			divContent += '<span style="' + this.labelFontFamily() + this.textSize() + this.textColor() + '">Drag & drop file here</span>';
			divContent += '<span style="' + this.labelFontFamily() + this.textSize() + this.subTextColor() + '">Limit 100MB per file</span>';
			divContent += "</div>";
			...
			document.styleSheets[0].addRule("#drag-area" + idWidget, " background-color: " + modelsParameters[idInstance].backgroundColor);
			document.styleSheets[0].addRule("#load-file-widget-html" + idWidget, " border-color: " + modelsParameters[idInstance].borderColor);
			document.styleSheets[0].addRule("#load-file-widget-html" + idWidget, " background-color: " + modelsParameters[idInstance].labelBackgroundColor);
			...
			this.addFileDiv = function (name, size) {
				...
				document.styleSheets[0].addRule("#close-file-btn" + idWidget + " > i", "color: " + modelsParameters[idInstance].deleteButtonIconColor);
				document.styleSheets[0].addRule("#close-file-btn" + idWidget + " > i:hover", "color: " + modelsParameters[idInstance].deleteButtonIconHoverColor);
			...
			if (!_.isUndefined(modelsTempParams[idInstance].fileName) && !_.isUndefined(modelsTempParams[idInstance].fileSize)) {
		...
		const _OUTPUT_DESCRIPTOR = new WidgetActuatorDescription("outputFile", "File content", WidgetActuatorDescription.FILE);
