...
function annotationWidgetsPluginClass() {
    ...
    this.iconInfoAnnotationWidget = function(idDivContainer, idWidget, idInstance, bInteractive) {
        ...
        const _VALUE_DESCRIPTOR = new WidgetActuatorDescription("value", "Hover text of the icon", WidgetActuatorDescription.READ, WidgetPrototypesManager.SCHEMA_STRING);
    ...
    this.labelAnnotationWidget = function(idDivContainer, idWidget, idInstance, bInteractive) {
        ...
        this.render = function() {
            ...
            var divContent = '<div id="annotationLabelTextArea' + idWidget +
        ...
        const _VALUE_DESCRIPTOR = new WidgetActuatorDescription("value", "Label text", WidgetActuatorDescription.READ, WidgetPrototypesManager.SCHEMA_ANY_PRIMITIVE);
        this.getActuatorDescriptions = function (model = null) {
            const data = model || modelsParameters[idInstance];
            return (data && data.enableActuator) ? [_VALUE_DESCRIPTOR] : [];
        };
    ...
    this.imageAnnotationWidget = function(idDivContainer, idWidget, idInstance, bInteractive) {
        ...
        this.render = function() {
            var widgetHtml = document.createElement('div');
            widgetHtml.setAttribute("style", "width: inherit; height: inherit; margin: auto; position: absolute; top: 0; left: 0"); // MBG 09/10/2019 : margin auto for centering
            ...
            if (modelsHiddenParams[idInstance].fileContentBase64 != "") {
                ...
                divContent = divContent + '" src="data:' + modelsHiddenParams[idInstance].mimeType + ';base64,' + modelsHiddenParams[idInstance].fileContentBase64 + '">'
                divContent = divContent + '</img>'
            }
            if (!self.bIsInteractive && !modelsParameters[idInstance].hideImageURL) {
                divContent = divContent + '<div id="image_drop_zone' + idWidget + '" style="z-index: 1000000; position: absolute; display: table; top: 20px; left:20px; height: calc(100%-40px); width: calc(100%-40px)">'
                divContent = divContent + '<div id="image_drop_zone_new' + idWidget + '" style="; cursor: pointer; display: table-cell; vertical-align: middle"><a><center id="image_import_handle' + idWidget + '">Click here to import your image file</center></a></div>';

                divContent = divContent + '</div>';
                var parent = document.getElementById('_otherUse');
                var newChild = '<input type="file" id="image-file' + idWidget + '" name="files[]" accept=".jpg,.jpeg,.png,.gif" style="padding-bottom: 15px; display: none" required="">';
            ...
            if (!self.bIsInteractive && !modelsParameters[idInstance].hideImageURL) {
                ...
                inputElt.addEventListener("click", function (e) {
                    ...
                    imageFileElt.addEventListener("change", function (e) {
                        e.stopPropagation();                       
                        ...
                        reader.addEventListener("load", function (event) {
                            ...
                            modelsHiddenParams[idInstance].fileContentBase64 = encodeURIComponent(base64ArrayBuffer(imageFile.result));
                            var i = new Image();
                            ...
                            i.src = 'data:' + modelsHiddenParams[idInstance].mimeType + ';base64,' + modelsHiddenParams[idInstance].fileContentBase64;
                            self.render();
    ...
    this.videoAnnotationWidget = function(idDivContainer, idWidget, idInstance, bInteractive) {
        ...
        this.screenshot = function() {
            ...
            ctx.drawImage(self.video, capture.x, capture.y, capture.width, capture.height, 0, 0, capture.width, capture.height);
        ...
        this.render = function() {
            ...
            let divContent = '<div id="videoContainer' + idWidget + '" ' +
                'style="text-align: center; align: center; width: inherit; height: inherit;">';
            divContent = divContent + '<div style="display: flex;">';
            divContent = divContent + '<div style="margin: 0 auto; position: relative; ' +
                'height: ' + (videoHeightPx + 2) + 'px; ' + videoStyle + '; flex: 100%;">';
            divContent = divContent + '<video id="video' + idWidget + '" ' +
                'class="videostream" ' +
                'style="position: absolute; top: 0; left: 0;" width="100%" ' +
                'height="' + videoHeightPx + 'px" autoplay></video>';
            divContent = divContent + '<canvas id="canvas' + idWidget + '" ' +
                'style="max-width: 100%; max-height: 100%; position: absolute; ' +
                'top: 0; left: 0; z-index: 10;"></canvas>';
            divContent = divContent + '</div>';
            divContent = divContent + '<div id="cropbar' + idWidget + '" ' +
                'style="flex: 5px;" hidden>' +
                '<div class="v-slider-div-div" id="slider-crop' + idWidget + '" ' +
                'style="height: ' + (videoHeightPx + 2) + 'px;"></div></div>';
            divContent = divContent + '</div>';

            divContent = divContent + '<div style="display:table; width:100%">' +
                '<div class="container"><div style="margin: 0px" class="row">';
            divContent = divContent + '<div class="col-3" style="padding: 1px">' +
                '<button id="video-capture-button-' + idWidget + '">' + START + '</button></div>';
            divContent = divContent + '<div class="col-6" style="padding: 1px">' +
                '<button id="video-screenshot-button-' + idWidget + '">screenshot</button></div>';
            divContent = divContent + '<div class="col-3" style="padding: 1px">' +
                '<button id="video-crop-button-' + idWidget + '">crop</button></div>';
        ...
        const _BASE64_DESCRIPTOR = new WidgetActuatorDescription("base64Image", "Sceenshot as a data URL", WidgetActuatorDescription.WRITE, WidgetPrototypesManager.SCHEMA_STRING);
        const _MIME_DESCRIPTOR = new WidgetActuatorDescription("mimeType", "MIME type of the screenshot", WidgetActuatorDescription.WRITE, WidgetPrototypesManager.SCHEMA_STRING);
        const _IMAGE_DESCRIPTOR = new WidgetActuatorDescription("imageData", "Sceenshot image raw data encoded as base 64", WidgetActuatorDescription.WRITE, WidgetPrototypesManager.SCHEMA_STRING);
    ...
    this.pluginDefinition = {
        ...
        'widgetsDefinitionList': {
            ...
            annotationLabel: { factory: "labelAnnotationWidget", title: "Label", icn: "label", help: "wdg/wdg-annotation-video/#label" },
            annotationImage: { factory: "imageAnnotationWidget", title: "Image", icn: "image", help: "wdg/wdg-annotation-video/#image" },
            annotationVideo: { factory: "videoAnnotationWidget", title: "Camera", icn: "camera", help: "wdg/wdg-annotation-video/#camera" }
        },
        // MBG : need to fix cohabitation with direct user input 
