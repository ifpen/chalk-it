...
modelsHiddenParams.openStreetMaps = {
    ...
    'legends': [],
    'colors': ['#800080', '#FF00FF', '#000080', '#0000FF', '#008080', '#00FFFF', '#FFFF00', '#800000',
        '#6666ff', '#cc6699', '#66ff66', '#666699', '#996633', '#ff0000', '#669999', '#ffff99'
    ]
};
...
const __SCHEMA_GEOJSON_COORD = {
    ...
    anyOf: [ // [longitude, latitude, elevation]
        {
            type: "array",
            items: [
                { type: "number", minimum: -180, maximum: 180 },
                    ...
                    then: {
                        properties: {
                            properties: {
                                ...
                                anyOf: [ //  comment and html are exclusive
...
function mapWidgetsPluginClass() {
    ...
    var arrowDefStr = '<defs><marker id="styledArrow" markerUnits="strokeWidth" markerWidth="12" markerHeight="12" viewBox="0 0 12 12" refX="6" refY="6" orient="auto"><path d="M2,2 L10,6 L2,10 L6,6 L2,2" style="fill: black;"></path></marker></defs>';
    ...
    this.openStreetMapsWidget = function (idDivContainer, idWidget, idInstance, bInteractive) {
        ...
        this.getGeoJsonPoint = function (typeLayers, Points) {
            ...
            if ((typeLayers.geometry.type === "polygon" || typeLayers.geometry.type === "Polygon") && typeLayers.geometry.isCut) {
                ...
                return ListPositions;
            } else if ((typeLayers.geometry.type === "polygon" || typeLayers.geometry.type === "Polygon") && !typeLayers.geometry.isCut) {
        ...
        this.getGeoJsonPoint1 = function (typeLayers, Points, isRectangle) {
            ...
            if (typeLayers === "rectangle" || (typeLayers === "Polygon" && isRectangle)) {
                ...
                points = [];
                ListPositions = [Points[1],
                Points[0]
                ];
            }

            for (var leng = 0; leng < points.length; leng++) {
                ListPositions.push([points[leng][1],
                points[leng][0]
                ]);
            }
            return ListPositions;
                ...
                for (var lay = 0; lay < modelsHiddenParams[idInstance].selectedGeoJson.features.length; lay++) {
                    if (modelsHiddenParams[idInstance].selectedGeoJson.features[lay]["properties"]["layerId"] == id) {
                        if (modelsHiddenParams[idInstance].selectedGeoJson.features[lay]["properties"]["information"]) {
                            ...
                            infos.description = modelsHiddenParams[idInstance].selectedGeoJson.features[lay]["properties"]["information"].description;
                            infos.timeStamp = modelsHiddenParams[idInstance].selectedGeoJson.features[lay]["properties"]["information"].timeStamp;
                        }
                    }
                }
                self.selectedGeoJson.updateCallback(self.selectedGeoJson, self.selectedGeoJson.getValue());
            ...
            if (modelsHiddenParams[idInstance].selectedGeoJson) {
                ...
                for (var r = 0; r < keysEdited.length; r++) {
                    for (var rr = 0; rr < modelsHiddenParams[idInstance].selectedGeoJson.features.length; rr++) {
                        if (modelsHiddenParams[idInstance].selectedGeoJson.features[rr]["properties"]["layerId"] == listEditedLayers[keysEdited[r]]._leaflet_id) {
        ...
        this.modal = function (options) {
            if (modelsParameters[idInstance].drawingFeaturesOptions.modal) {
                ...
                var _tmp = ['<div class="modal-header"><h4>{title} </h4></div>',
        ...
        this.addDrawingFeatures = function () {
            ...
            if (this.bIsInteractive) {
                ...
                self.map.on('pm:cut', function (e) {
                    ...
                    var my_leaflet_id = e.originalLayer._drawnByGeoman ? e.originalLayer._leaflet_id : e.originalLayer.feature.properties.layerId;
                    for (var rr = 0; rr < modelsHiddenParams[idInstance].selectedGeoJson.features.length; rr++) {
                        if (modelsHiddenParams[idInstance].selectedGeoJson.features[rr]["properties"]["layerId"] == my_leaflet_id) {
                            modelsHiddenParams[idInstance].selectedGeoJson.features[rr].properties.layerId = e.layer._leaflet_id;
                            modelsHiddenParams[idInstance].selectedGeoJson.features[rr].geometry.coordinates = e.layer.feature.geometry.coordinates;
        ...
        this.render = function () {
            ...
            self.map = L.map('openStreetMap' + idWidget, { preferCanvas: true }).setView([modelsParameters[idInstance].defaultCenter.latitude, modelsParameters[idInstance].defaultCenter.longitude], modelsParameters[idInstance].defaultCenter.zoom);
            ...
            if (!_.isUndefined(modelsParameters[idInstance].tileServer)) {
                ...
                } else { /* TODO : good error message at edit mode to send */ }
            }
            ...
            if (modelsParameters[idInstance].offlineSupport) {
                tileConf.attribution = 'Progress: <span id="leafletProgress' + idWidget + '"></span> / ' +
                    '<span id="leafletTotal' + idWidget + '"></span>. ' +
                    'Current storage: <span id="leafletStorage' + idWidget + '"></span> files.' +
                    'Map data &copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors, ' +
                    '<a href="http://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, ' + tileServers[ts].attribution;
            ...
            try {
                ...
                for (var i = 0; i < self.numberOfGeoJsonLayers; i++) {
                    if ((!_.isUndefined(modelsHiddenParams[idInstance].geoJson.geoJsonLayers[i])) && (!_.isUndefined(modelsHiddenParams[idInstance].geoJson.geoJsonLayers[i]))) {
                        self.addGeoJson(modelsHiddenParams[idInstance].geoJson.geoJsonLayers[i], i + 1);
                    }
                }

                for (var i = 0; i < self.numberOfChoroplethLayers; i++) {
                    if ((!_.isUndefined(modelsHiddenParams[idInstance].choropleth.choroplethLayers[i])) && (!_.isUndefined(modelsHiddenParams[idInstance].choropleth.choroplethLayers[i]))) {
                ...
                for (var i = 0; i < self.numberOfLineHeatMapLayers; i++) {
                    if ((!_.isUndefined(modelsHiddenParams[idInstance].lineHeatMap.lineHeatMapLayers[i])) && (!_.isUndefined(modelsHiddenParams[idInstance].lineHeatMap.lineHeatMapLayers[i]))) {
                ...
                for (var i = 0; i < self.numberOfHeatMapLayers; i++) {
                    ...
                    if ((!_.isUndefined(modelsHiddenParams[idInstance].heatMap.heatMapData[i])) && (!_.isUndefined(modelsHiddenParams[idInstance].heatMap.heatMapData[i]))) {
                        self.drawHeatMap(modelsHiddenParams[idInstance].heatMap.heatMapData[i], i + 1);
                    }
                }

                for (var i = 0; i < self.numberOfImageLayers; i++) {
                    if ((!_.isUndefined(modelsHiddenParams[idInstance].imageOverlay.imageData[i])) && (!_.isUndefined(modelsHiddenParams[idInstance].imageOverlay.imageData[i]))) {
                        self.addImageOverlay(modelsHiddenParams[idInstance].imageOverlay.imageData[i], i + 1);
                    }
                }

                for (var i = 0; i < self.numberOfSvgLayers; i++) {
                    if ((!_.isUndefined(modelsHiddenParams[idInstance].svgOverlay.svgData[i])) && (!_.isUndefined(modelsHiddenParams[idInstance].svgOverlay.svgData[i]))) {
                    ...
                    if (feature.geometry.type == 'Point' && feature.properties) {
                        if (feature.properties.comment) {
                            if (feature.properties.openPopup) {
                                let mk = layer.bindPopup(feature.properties.comment,
                            ...
                            if (feature.properties.html) {
                                if (feature.properties.openPopup) {
                                    mk = layer.bindPopup(feature.properties.html,
                                        { autoClose: false, autoPan: false });
                                    mk.on("add", function (event) {
                                        event.target.openPopup();
                                    });
        ...
        this.addChoropleth = function (choroplethData, layerIndex) {
            ...
            try {
                if (!_.isUndefined(choroplethData.config.max))
                    maxValue = choroplethData.config.max;
                if (!_.isUndefined(choroplethData.config.min))
                    minValue = choroplethData.config.min;
                if (!_.isUndefined(choroplethData.config.weight))
        ...
        this.addLineHeatMap = function (lineHeatMap, layerIndex) {
            ...
            try {
                if (!_.isUndefined(lineHeatMap.config.max))
                    maxValue = lineHeatMap.config.max;
        ...
        this.addImageOverlay = function (imgStruct, layerIndex) {
            ...
            self.mapLayers.imageOverlay.imageLayers[layerIndex - 1] = L.imageOverlay(imageUrl, imageBounds)
                .addTo(self.map);

            if (addAs == 'overlay')
                self.ctrl.addOverlay(self.mapLayers.imageOverlay.imageLayers[layerIndex - 1], featureTitle);
            else
                self.ctrl.addBaseLayer(self.mapLayers.imageOverlay.imageLayers[layerIndex - 1], featureTitle);
        };
        ...
        this.appendArrowsOverlay = function (svgStruct, i) {
            ...
            var arrow = self.createOneArrow(x, y, direction,
                modelsHiddenParams[idInstance].svgOverlay.svgElements[i - 1].reductionFactor,
                modelsHiddenParams[idInstance].svgOverlay.svgElements[i - 1].lengthInPx, strokeColor);
        ...
        this.createOneArrow = function (xCenterPixels, yCenterPixels, direction, reductionFactor, lengthInPx, strokeColor) {
            ...
            var lineStr = '<line x1="' + xCenterPixels + '" y1="' + yCenterPixels + '" x2="' + xCenterPixels + '" y2="' + yEdegePixels + '" stroke="#000" stroke-width="' + 50 * reductionFactor + '" marker-end="url(#styledArrow)" ';
        ...
        this.createOneCircle = function (xCenterPixels, yCenterPixels, direction, reductionFactor, lengthInPx) {
            var lineStr = '<circle cx="' + xCenterPixels + '" cy="' + yCenterPixels + '" stroke="#000" r="' + 50 * reductionFactor + '"';
            lineStr = lineStr + '/>';
            return lineStr;
        }
        ...
        this.getTrianglePoints = function (depart, lenght, direction) {
            ...
            return firstPoint[1] + " " + firstPoint[0] + " " + secondPoint[1] + " " + secondPoint[0] + " " + thirdPoint[1] + " " + thirdPoint[0];
        }

        this.createOneTriangle = function (yCenterPixels, xCenterPixels, direction, reductionFactor, lengthInPx, type, color) {
            ...
            var trigl = '<polygon points="' + point + '"  style="fill:' + color + ';stroke:' + color + ';stroke-width:' +
                50 * reductionFactor + '"   />';
            if (type == "emptyTriangle") {
                trigl = '<polygon points="' + point + '"  fill="none" style="stroke:' + color + ';stroke-width:' +
                    50 * reductionFactor + '"   />';
            }
            return trigl;
        }
        ...
        this.addSvgOverlay = function (svgStruct, layerIndex, bAppend) {
            ...
            if (bAppend) {
                ...
                self.mapLayers.svgOverlay.svgLayers[layerIndex - 1] = L.layerGroup().addLayer(self.createSvgOverlay(svgStruct, layerIndex))
                    .addTo(self.map);

                if (addAs == 'overlay')
                    self.ctrl.addOverlay(self.mapLayers.svgOverlay.svgLayers[layerIndex - 1], featureTitle);
                else
                    self.ctrl.addBaseLayer(self.mapLayers.svgOverlay.svgLayers[layerIndex - 1], featureTitle);
            }
        ...
        this.drawHeatMap2 = function (heatMapObject, layerIndex) {
            ...
            for (k = 0; k < formattedHeatMap.length; k++) {
                ...
                circles.push(L.circle([lats[k], lngs[k]], {
                    stroke: false,
                    color: currentColor,
                ...
                if (addTextLabel) {
                    circles.push(L.marker(new L.LatLng(lats[k], lngs[k]), { icon: self.createLabelIcon("boldWhiteHeatMapLabel", nFormatter(formattedHeatMap[k][2])) }));
                }
            }
            ...
            if (_.isEmpty(self.mapLayers.heatMap.heatMapLayers[layerIndex - 1])) {
                ...
                if (_.isUndefined(modelsParameters[idInstance].heatMap.densityHeatMap)) {
                    ...
                    if (modelsHiddenParams[idInstance].heatMap.densityHeatMap) {
                        ...
                        const sameConfig = (configNew.circleFillOpacity === undefined || configNew.circleFillOpacity === configOld.circleFillOpacity)
                        ...
                        if (sameConfig) {
                            let normVal = 100 * (hmp[2] - self.configStore[layerIndex - 1].minValue) / (self.configStore[layerIndex - 1].maxValue - self.configStore[layerIndex - 1].minValue);
                            ...
                            const heatMapToRedraw = { "data": modelsHiddenParams[idInstance].heatMap.heatMapBuffer[layerIndex - 1], "config": heatMapPoint.config }
        ...
        const _SELECTED_POINT_DESCRIPTOR = new WidgetActuatorDescription("selectedPoint", "Selected coordinates", WidgetActuatorDescription.WRITE, _SCHEMA_SELECTED_POINT);
        const _SELECTED_GEOJSON_DESCRIPTOR = new WidgetActuatorDescription("selectedGeoJson", "Drawn GeoJSON figure", WidgetActuatorDescription.WRITE, _SCHEMA_SELECTED_GEOJSON);
            ...
            if (data) {
                ...
                if (data.geoJson && data.geoJson.numberOfLayers) {
                    for (let i = 1; i <= data.geoJson.numberOfLayers; i++) {
                        result.push(new WidgetActuatorDescription('geoJson' + i, "GeoJSON features", WidgetActuatorDescription.READ, _SCHEMA_GEOJSON));
                    }
                }

                if (data.lineHeatMap && data.lineHeatMap.numberOfLayers) {
                    for (let i = 1; i <= data.lineHeatMap.numberOfLayers; i++) {
                        result.push(new WidgetActuatorDescription('lineHeatMap' + i, "Lines colored by values", WidgetActuatorDescription.READ, _SCHEMA_LINE_HEATMAP));
                    }
                }

                if (data.choropleth && data.choropleth.numberOfLayers) {
                    for (let i = 1; i <= data.choropleth.numberOfLayers; i++) {
                        result.push(new WidgetActuatorDescription('choropleth' + i, "Choropleth data", WidgetActuatorDescription.READ, _SCHEMA_CHOROPLETH));
                    }
                }
                    ...
                    for (let i = 1; i <= data.heatMap.numberOfLayers; i++) {
                        if (data.heatMap.sampledDisplay) {
                            ...
                            if(data && !data.densityHeatMap) {
                                result.push(new WidgetActuatorDescription(name, "Cumulative heatmap", WidgetActuatorDescription.READ, _SCHEMA_HEATMAP_SAMPLED));
                            } else {
                                result.push(new WidgetActuatorDescription(name, "Cumulative density heatmap", WidgetActuatorDescription.READ, _SCHEMA_HEATMAP_DENSITY_SAMPLED));
                            }
                        } else {
                            const name = 'heatMapData' + i; 
                            if(data && !data.densityHeatMap) {
                                result.push(new WidgetActuatorDescription(name, "Heatmap", WidgetActuatorDescription.READ, _SCHEMA_HEATMAP));
                            } else {
                                result.push(new WidgetActuatorDescription(name, "Density heatmap", WidgetActuatorDescription.READ, _SCHEMA_HEATMAP_DENSITY));
                            }
                        }
                    }
                }
                ...
                if (data.imageOverlay && data.imageOverlay.numberOfLayers) {
                    for (let i = 1; i <= data.imageOverlay.numberOfLayers; i++) {
                        result.push(new WidgetActuatorDescription('imageOverlay' + i, "Image to place on the map", WidgetActuatorDescription.READ, _SCHEMA_IMAGE_OVERLAY));
                    }
                }
                ...
                if (data.svgOverlay && data.svgOverlay.numberOfLayers) {
                    for (let i = 1; i <= data.svgOverlay.numberOfLayers; i++) {
                        if (data.svgOverlay.sampledDisplay) {
                            result.push(new WidgetActuatorDescription('svgPoint' + i, "Place a symbol on the map", WidgetActuatorDescription.READ, _SCHEMA_SVG_POINT_OVERLAY));
                        } else {
                            result.push(new WidgetActuatorDescription('svgOverlay' + i, "Place symbols on the map", WidgetActuatorDescription.READ, _SCHEMA_SVG_OVERLAY));
                        }
                    }
                }
            }
    ...
    this.pluginDefinition = {
        ...
        'widgetsDefinitionList': {
            openStreetMaps: { factory: "openStreetMapsWidget", title: "Leaflet maps", icn: "map", help: "wdg/wdg-geo-time/#leaflet-maps" }
        },
    };
