...
function datePickerWidgetsPluginClass() {
    ...
    this.datePickerWidget = function (idDivContainer, idWidget, idInstance, bInteractive, mode) {
        ...
        this.render = function () {
            ...
            widgetHtml.setAttribute("style", displayTable + 'text-align: left; height: inherit; width: inherit; cursor: inherit');
            ...
            var divDatapicker = '<datepicker id="ng-datepicker' + idWidget + '" date-format="yyyy-MM-dd" datepicker-show="' + visibility + '" selector="form-control">' +
                ...
                ' " ' +
                'ng-model="' + modelObj + '.currentDate" ' +
                dataPickerdisabled +
                ' placeholder="Choose a date"/>' +
                '<span class="form-control-feedback" style="cursor: inherit; display: table;' +
                'margin-top:0; top:0; ' +
                'padding-right: ' + valueFontSize + "; height: " + heightCalender + 'px;' +
                '"><i class="fa fa-lg fa-calendar" style="display: table-cell; vertical-align: middle;' +
                ' font-size:' + valueFontSize + '"></i>' +
                '</span>' +
                '</div>' +
                '</datepicker>';
            var divContentTemplate =
                LabelHtml +
                '<div style="cursor: inherit; height: 100%"> ' +
                divDatapicker +
                '</div> ';
            ...
            if (visibility) {
                ...
                divContentTemplate = '<div style="' + styleL + '"> ' +
                    divDatapicker +
                    '</div> ' +
                    LabelHtml;
            }
            ...
            angular.element(document.body).injector().invoke(['$compile', '$rootScope', function ($compile, $rootScope) {
                ...
                $rootScope.$watch("datepickerValues.val" + idInstance, function () {
                    self.updateValue();
                }, true);
    ...
    this.pluginDefinition = {
        ...
        'widgetsDefinitionList': {
            datepickerSimple: { factory: "datePickerWidgetSemi", title: "Simple calender", icn: "calendar", help: "wdg/wdg-geo-time/#simple-calendar" }
        },
    };
