...
function flatUiAddressCompletionWidgetsPluginClass() {
    ...
    this.hereAutocompleteValueFlatUiWidget = function (idDivContainer, idWidget, idInstance, bInteractive) {
        ...
        this.initTypeahead = function () {
            ...
            $("#ac-value" + idWidget).typeahead(null, {
                ...
                source: function (query, result) {
                    $.ajax({
                        url: "https://xdashgateway.azure-api.net/autocomplete/suggest.json" + "?query=" + query + "&language=fr" + country,
                        method: "GET",
                        dataType: "json",
                        success: function (data) {
                            var tab = self.formatHereSuggestTable(data);
                            var words = self.formatSuggestWords(tab);
                            if (!_.isUndefined(words)) //AEF: fix bug here because when length of tab==0 formatSuggestWords returned undefined 
                                result($.map(words, function (item) { return item; }));
                        }
                    });
                }
            });
        ...
        const _VALUE_DESCRIPTOR = new WidgetActuatorDescription("value", "Current value", WidgetActuatorDescription.READ_WRITE, WidgetPrototypesManager.SCHEMA_STRING);
