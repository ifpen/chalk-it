...
function dateRangePickerWidgetsPluginClass() {
  ...
  this.dateRangePickerWidget = function (idDivContainer, idWidget, idInstance, bInteractive) {
    ...
    this.render = function () {
      ...
      const daterangepickerHtml =
        '<div id="daterangepicker' +
        idWidget +
        '" >' +
        '<input type="text" autocomplete="off" ' +
        dateRangePickerDisabled +
        ' placeholder="Choose a date range" id="daterangepickerInput' +
        idWidget +
        '" ' +
        'style="' +
        cursorIcon +
        ...
        '</div>';
      const divContent =
        LabelHtml + '<div style="height:100% ' + 'width: ' + widthInput + 'px;">' + daterangepickerHtml + '</div>';
      widgetHtml.innerHTML = divContent;

      if (modelsParameters[idInstance].displayTimePicker) {
        self.formatDateValue = modelsParameters[idInstance].displayTimePicker24Hour
          ? 'YYYY-MM-DD hh:mm'
          : 'YYYY-MM-DD hh:mm A';
      } else {
    ...
    const _DATE_START_DESCRIPTOR = new WidgetActuatorDescription(
      'startDateValue',
      'Selected start date as YYYY-MM-DD',
      WidgetActuatorDescription.READ_WRITE,
      _DATE_SCHEMA
    );
    const _DATE_END_DESCRIPTOR = new WidgetActuatorDescription(
      'endDateValue',
    ...
    this.startDateValue = {
      ...
      setValue: function (val) {
        if (moment(val, self.formatDateValue, true).isValid()) {
          ...
          $('input[id="daterangepickerInput' + idWidget + '"]')
            .data('daterangepicker')
            .setStartDate(modelsHiddenParams[idInstance].startDateValue);
        }
      },
      getValue: function () {
    ...
    this.endDateValue = {
      ...
      setValue: function (val) {
        if (moment(val, self.formatDateValue, true).isValid()) {
          ...
          $('input[id="daterangepickerInput' + idWidget + '"]')
            .data('daterangepicker')
            .setEndDate(modelsHiddenParams[idInstance].endDateValue);
        }
      },
      getValue: function () {
  ...
  this.pluginDefinition = {
    ...
    widgetsDefinitionList: {
      dateRangePicker: {
        factory: 'dateRangePickerWidget',
        title: 'Date-range calendar',
        icn: 'date-range-calendar',
        help: 'wdg/wdg-geo-time/#advanced-calendar',
      },
    },
  };
