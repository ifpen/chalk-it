...
export function legends() {
  function createLegend(idLegend, color, length, colorStops, min_, max_, featureTitle) {
  ...
  function createChoroplethLegend(idLegend, getColor, min_, max_, featureTitle, colorScale) {
    if (min_ == max_) return;
    if (_.isUndefined(featureTitle) || featureTitle == 'none') return;
    featureTitle = featureTitle ? featureTitle.charAt(0).toUpperCase() + featureTitle.toLowerCase().slice(1) : '';
    var legend = L.control({ position: 'topleft' });
  ...
  function toggleLegend(self, layerIndex, styleForObject, geoJSONinLayer) {
