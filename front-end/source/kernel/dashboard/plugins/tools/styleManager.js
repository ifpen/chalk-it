...
export function styleManager() {
  function createTemplateStyle(self, geoJSON, index, typeLayer = undefined) {
    let prop = geoJsonTools.findPropertiesWithNumber(geoJSON);
    let allProp = geoJsonTools.findAllProperties(geoJSON);
    let JSONtype = geoJsonTools.findFeatureType(geoJSON);
    let baseStyle = {
      type: JSONtype,
      layer: 'layer ' + (index + 1),
      name: 'layer ' + (index + 1),
  ...
  function updateLayerStyle(self, layer, styleForObject, geoJSONinLayer) {
  ...
  function setStyle(self, layerIndex, style) {
    // Get GeoJSON
