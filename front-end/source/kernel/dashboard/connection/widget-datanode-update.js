...
function displaySpinnerOnInputFileButton(idInstance, idWidget) {
    ...
    var disableButton = function() { // disable until request finished
        ...
        if (dsNames.length > 0) {
            ...
            var intervalId = setInterval(function() {
                ...
                dsNames.forEach(element => {
                    if (datanodesManager.getDataNodeByName(element).status() == "Pending") { // check if datanode is in Pending state
