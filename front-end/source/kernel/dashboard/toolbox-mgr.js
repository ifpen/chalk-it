...
function widgetToolboxClass() {
    ...
    var widgetToolbox = function () {
        ...
        function fillCollapseHeader(wdgtGroupId, wdgtGroupName, index) {
            var html = [
                ...
                '    <div ng-click="toggleWidgetLibDisplay(' + index + ')" class="widget__groups--wrapper--top btn--collapse" id="' + wdgtGroupId[i] + '">',
    ...
    $("#inputSearchWidget").on("input", function (e) {
        ...
        for (let k = 0; k < allWidgets.length; k++) {
            if (val.length > 0 && allWidgets[k].title.toLowerCase().match(val) && widgetsToSearch.indexOf(allWidgets[k].title) === -1) {
