...
function widgetContainerClass() {
    ...
    this.putAndGetTargetDiv = function (cln, targetDiv) {
        if (!targetDiv) {
            targetDiv = layoutMgr.getDefaultContainer();
        }
    ...
    this.computeAndApplyContainerDivLayout = function (cln, div) {
        ...
        var w = parseFloat(cln.style.width.substring(0, cln.style.width.length - 2)) -
            (2 * (100 / document.documentElement.clientWidth));
        var h = parseFloat(cln.style.height.substring(0, cln.style.height.length - 2)) -
        ...
        if (cln.style.minWidth != "") {
            ...
            div.style.width = Math.max((parseFloat(div.style.minWidth)) *
                (100 / document.documentElement.clientWidth), parseFloat(div.style.width)) + 'vw';
        }
        if (cln.style.minHeight != "") {
            div.style.minHeight = parseFloat(cln.style.minHeight) - 5 + 'px';
            div.style.height = Math.max((parseFloat(div.style.minHeight)) *
                (100 / document.documentElement.clientHeight), parseFloat(div.style.height)) + 'vh';
        }
    }
    ...
    this.replaceWidget = function (element) {
        ...
        widgetEditor.widgetObject[instanceId] = widgetsPluginsHandler.copyWidget(wcId, modelJsonIdStr, widgetEditor.widgetObject[instanceId], instanceId, false);
    ...
    this.highlightWidgets = function _highlightWidgets(elementIds) {
        elementIds.forEach(id => {
            const elem = widgetEditor.widgetContainers.get(id);
...
widgetContainer = new widgetContainerClass();