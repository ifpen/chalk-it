...
window.addEventListener('click', (event) => {
    ...
    if (scopeDash.editorView.newDatanodePanel.view) { //AEF: unselect widget only when editing datanodes
        if (event.target.className !== "widget-overlay") {
            widgetEditor.unselectAllWidgets();
        }
    }

    if (event.target.id !== "showHideWidgetMenu" && event.target.className !== "introjs-button introjs-nextbutton" && event.target.className !== "introjs-button introjs-prevbutton" && event.target.className !== "introjs-showElement") {
    ...
    if (angular.element(document.getElementById('my-personnal-projects-list')).scope()) {
        ...
        if (!_.isUndefined(angular.element(document.getElementById('my-personnal-projects-list')).scope().allFilesFiltred[0]))
            counter = angular.element(document.getElementById('my-personnal-projects-list')).scope().allFilesFiltred[0].FileList.length;

        for (var i = 0; i < counter; i++) {
            var isNavOpen_Recent_All_Gallery = angular.element(document.getElementById('isNavOpen_Recent_All_Gallery' + i)).scope();
