...
function initEditWidget() {
    ...
    if ($('#DropperDroite')[0].hasChildNodes()) {
        ...
        for (var i = L - 1; i >= 0; i--) {
            if (div.childNodes[i].id != "menuWidget")
        ...
        .resizable({
            ...
            listeners: {
                ...
                move(event) {
                    ...
                    if (resizes.size > 0) {
                        angular.element(document.body).injector().invoke(['UndoManagerService', 'EditorActionFactory', (undoManagerService, editorActionFactory) => {
                            const action = editorActionFactory.createSetGeometryAction(resizes, "Resize widget" + (resizes.size > 1 ? "s" : ""));
                            undoManagerService.execute(action);
                        }]);
                    }
                },
                ...
                move(event) {
                    ...
                    widgetSelectionContext.forEach(id => {
                        ...
                        if (container === MAIN_CONTAINER_ID) {
                            ...
                            if (grid) {
                                [toPosition.left, toPosition.top] = _alignOnGrid([toPosition.left, toPosition.top], [containerLayout.left, containerLayout.top], grid);
                            }
                    ...
                    if (moves.size > 0) {
                        angular.element(document.body).injector().invoke(['UndoManagerService', 'EditorActionFactory', (undoManagerService, editorActionFactory) => {
                            const action = editorActionFactory.createDropToCellAction(container, moves, "Move widget" + (moves.size > 1 ? "s" : ""));
                            undoManagerService.execute(action);
                        }]);
                    }
                },
    ...
    function _onAddRmWidget() {
        angular.element(document.body).injector().invoke(['EventCenterService', (eventCenterService) => {
    ...
    function serialize() {
        ...
        for (const [key, val] of widgetContainers) {
            ...
            if (bNoteBookMode || val.modelJsonId == 'annotationLabel' ||
                val.modelJsonId == 'annotationImage') {
                dashJson[key] = {
                    'layout': wcLayout,
                    'container': container,
    ...
    function deserialize(dashObj, scalingObj, deviceObj) {
        ...
        if (_.isUndefined(scalingObj)) {
            ...
            /*------------------------------------------------------------------------*/
            var mediaChangeProj = scalingHelper.mediaChangeProjection(scalingObj,
                editorDimensionsSnapshot, layoutMgr.getRows());
            /*------------------------------------------------------------------------*/
    ...
    function computeWidgetsRatio(instanceId) {
        if (!_.isUndefined($("#" + instanceId)[0])) { //AEF: fix bug, must be to be tested if creation has been failed
            widthRatioModels[instanceId] = $("#" + instanceId).width() /
                $("#" + $("#" + instanceId)[0].parentNode.parentNode.id).width();
            heightRatioModels[instanceId] = $("#" + instanceId).height() /
                $("#" + $("#" + instanceId)[0].parentNode.parentNode.id).height();
            leftRatioModels[instanceId] = $("#" + instanceId).position().left /
                $("#" + $("#" + instanceId)[0].parentNode.parentNode.id).width();
            topRatioModels[instanceId] = $("#" + instanceId).position().top /
                $("#" + $("#" + instanceId)[0].parentNode.parentNode.id).height();
    ...
    function showHideWidgMenu(event) {
        ...
        if ((nbRows <= 1) && ((containerOffsetTop + elementOffsetTop + menuWidgetMarginTop + menuWidgetHeight) >=  mainContainerOffsetHeight) && ((elementOffsetBottom + menuWidgetMarginBottom + menuWidgetHeight) >= mainContainerOffsetHeight)) {
            ...
            $("#" + idList)[0].style.top = "auto";
            document.styleSheets[0].addRule("#menuWidget::after", "top: " + menuWidgetHeight + "px; transform: rotate(180deg)");
        } else {
        ...
        if (((elementWidth + elementOffsetLeft).toString() <= menuWidgetWidth) && (containerOffsetLeft == 0)) {
            ...
            $("#" + idList)[0].style.left = (elementWidth + elementOffsetLeft + containerOffsetLeft - menuWidgetWidth).toString() + "px";
