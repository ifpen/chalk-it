...
var widgetPreview = (function() {
    ...
    var targetRows = 0;
    var targetCols = 1;
    var scalingHelper = new rescaleHelper(previewDimensionsSnapshot,
        targetScalingMethod, 'preview');
        ...
        for (let propName in widget) {
            for (let sliderName of widgetConnector.effectiveSliders()) {
                if (widget[propName] != null) {
                    if (widget[propName].getByName(sliderName) != null) { //getting slider object
    ...
    function deserialize(connectObj) {
        ...
        _.each(_.keys(connectObj), (instanceId) => {
            ...
            if (widgetConnector.widgetsConnection[instanceId] == null) { //Add connection  
        ...
        for (key in widgetConnector.widgetsConnection) {
            for (var actuator in connectObj[key]) {
                if (_.isUndefined(connectObj[key][actuator].dataNode)) { //compatibility
        ...
        for (key in widgetConnector.widgetsConnection) {
            try { // MBG temporary fix to handle pb of long requests
            ...
            for (var i in widgetConnector.widgetsConnection[instanceId].sliders) {
                if (widgetConnector.widgetsConnection[instanceId].sliders[i].name != "None") {
                    ...
                    if (widgetConnector.widgetsConnection[instanceId].widgetObjEdit != null) {
                        actuator = widgetConnector.widgetsConnection[instanceId].widgetObjEdit.getByName(widgetConnector.widgetsConnection[instanceId].sliders[i].name);
                    } else if (widgetConnector.widgetsConnection[instanceId].widgetObjConnect != null) {
                        actuator = widgetConnector.widgetsConnection[instanceId].widgetObjConnect.getByName(widgetConnector.widgetsConnection[instanceId].sliders[i].name);
                    }

                    if (actuator != null) {
                        if (widgetConnector.widgetsConnection[instanceId].sliders[i].dataNode != "None") {
                            var index = widgetConnector.widgetsConnection[instanceId].sliders[i].dataNodeIndex;
                            if (index != -1) {
                                var newData = datanodesManager.getAllDataNodes()[index].latestData();
                                var dnName = datanodesManager.getAllDataNodes()[index].name();
                                var status = datanodesManager.getAllDataNodes()[index].status();
                                var last_updated = datanodesManager.getAllDataNodes()[index].last_updated();
        ...
        if (!widgetsErrorState[instanceId]) {
            ...
            if (!_.isUndefined($("#" + divId)[0])) {
                if ($("#" + divId)[0].style.border != "3px groove #e40000") {
                    ...
                    $("#" + divId).append('<br/><span style="color:red">' + msg + ' "' + widgetConnector.widgetsConnection[instanceId].sliders[i].dataNode + '" !</span>');
                }
            }
    ...
    function removeDisplayErrorOnWidget(instanceId) {
        var divId = widgetConnector.widgetsConnection[instanceId].instanceId + 'c';
        if (!_.isUndefined($("#" + divId)[0])) {
            if ($("#" + divId)[0].style.border != "1px solid rgba(255, 255, 255, 0)") { //AEF: fix border
                w = parseFloat($("#" + divId)[0].style.width) - 1;
                h = parseFloat($("#" + divId)[0].style.height) - 1;
                $("#" + divId)[0].style.width = w + 'vw';
                $("#" + divId)[0].style.height = h + 'vh';
                ...
                for (var j = 0; j < $("#" + divId)[0].childNodes.length; j++) {
                    if ($("#" + divId)[0].childNodes[j].localName == "span")
                        $("#" + divId)[0].childNodes[j].remove();
                }
            }
        }
        widgetsErrorState[instanceId] = false;
    ...
    function setDataOnWidget(instanceId, i, actuator, newData, status, last_updated, bCaptionManuallyChanged) {
        if ((_.isUndefined(newData) && (last_updated != "never")) || status == "Error") { // MBG 
            msg = "Invalid data";
            displayErrorOnWidget(instanceId, i, msg);
        ...
        if (!(_.isUndefined(newData) || _.isNull(newData))) { // ABK: fix bug after MBG modif of &&(status!="None") // MBG 30/10/2018 add check on isNull following test "Gecoair UserTripso no JWT (6).html"
            ...
            for (let deep = 0; deep < slider.dataFields.length; deep++) {
                ...
                if (!isRange) { // pass through ranges
                    varName = field;
                    if (_.isNull(varInter) || _.isUndefined(varInter)) { // TODO ???
            ...
            if (varInter === null) {
                ...
                if (slider.name.substring(0, 1) === "D") { //for Digits
                    try {
    ...
    function clearDataFromWidget(instanceId, i, actuator, bCaption) {
        if (widgetConnector.widgetsConnection[instanceId].sliders[i].name.substring(0, 1) === "D") { //for Digits
            actuator.setText("");
        } else {
    ...
    function updateDataFromWidget(sender, e, doubleTrig) {
        var actuator = null;
        if (datanodesManager.getAllDataNodes().length == 0) {} else {
            var bFound = false;
            for (var elm in widgetConnector.widgetsConnection) {
                for (var i in widgetConnector.widgetsConnection[elm].sliders) {
                    if (widgetConnector.widgetsConnection[elm].widgetObjConnect != null) {
                        if (widgetConnector.widgetsConnection[elm].widgetObjConnect.getByName(widgetConnector.widgetsConnection[elm].sliders[i].name) != null) {
                            if (sender == widgetConnector.widgetsConnection[elm].widgetObjConnect.getByName(widgetConnector.widgetsConnection[elm].sliders[i].name)) {
            ...
            if (bFound) {
                if (actuator.dataNode != "None") {
                    var alldataNodes = datanodesManager.getAllDataNodes()[actuator.dataNodeIndex];
                    if (!_.isUndefined(alldataNodes)) {
                        if (alldataNodes.isSetValueValid() && !_.isUndefined(sender.getValue)) { // MBG 12/05/2017
                            try {
                                alldataNodes.setValue(actuator.dataFields, sender.getValue(), doubleTrig);
                            } catch (exc) {
                                console.log("setValue got exception with data: " + alldataNodes.name() + ". " + exc.message);
                            }
                        } else {
                            console.log("setValue is not possible with data: " + alldataNodes.name());
                        }
                    } else {
    ...
    return {
        ...
        getSnapshotDashZoneDims: function() { return previewDimensionsSnapshot; },
        ...
        getCols: function() { return scalingHelper.getCols(); },
