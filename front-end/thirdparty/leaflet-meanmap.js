...
*/
L.MeanLayer = (L.Layer ? L.Layer : L.Class).extend({
        ...
        for (i = 0, len = this._latlngs.length; i < len; i++) {
            ...
            if (bounds.contains(p)) {
                ...
                if (!cell) {
					...
					/*kk=Math.max(k,cell[2]);
                    cell[0] = (cell[0] * cell[2] + p.x * kk) / (cell[2] + kk); // x
                    cell[1] = (cell[1] * cell[2] + p.y * kk) / (cell[2] + kk); // y
                    cell[2] = Math.max(k,cell[2]); // cumulated intensity value
                    cell[3]+=1;*/
...
L.meanLayer = function (latlngs, options) {
    return new L.MeanLayer(latlngs, options);
};
